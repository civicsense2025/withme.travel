...
const ACTIVITY_CATEGORIES = {
  CULTURE: ['museum', 'history', 'art', 'gallery', 'theater', 'performance', 'exhibition', 'landmark', 'heritage', 'culture', 'historical'],
  NATURE: ['park', 'garden', 'trail', 'hike', 'mountain', 'beach', 'lake', 'river', 'forest', 'nature', 'outdoor', 'landscape', 'wildlife'],
  FOOD: ['restaurant', 'cafe', 'eatery', 'culinary', 'food', 'cuisine', 'dinner', 'lunch', 'breakfast', 'brunch', 'tasting', 'market'],
  ADVENTURE: ['adventure', 'sport', 'activity', 'rafting', 'climbing', 'surfing', 'diving', 'paragliding', 'kayaking', 'biking', 'cycling'],
  RELAXATION: ['spa', 'relax', 'massage', 'wellness', 'retreat', 'meditation', 'yoga', 'hot spring', 'thermal', 'chill'],
  SHOPPING: ['shop', 'market', 'mall', 'boutique', 'store', 'shopping', 'souvenir', 'craft', 'fair', 'bazaar', 'local goods'],
  NIGHTLIFE: ['bar', 'club', 'pub', 'lounge', 'nightlife', 'night', 'cocktail', 'entertainment', 'music', 'dance', 'concert'],
  TRANSPORT: ['tour', 'cruise', 'ferry', 'train', 'tram', 'cable car', 'scenic drive', 'boat', 'bicycle', 'rental']
};
...
const STOP_WORDS = new Set([
  ...
  'in', 'into', 'during', 'before', 'after', 'above', 'below', 'from', 'up', 'down',
  'with', 'about', 'against', 'between', 'through', 'throughout',
...
const ACTIVITY_TYPE_MAP: Record<string, string[]> = {
  ...
  'LANDMARK': ['landmark', 'monument', 'statue', 'tower', 'palace', 'castle', 'cathedral', 'church', 'temple'],
...
function generateActivityTitle(
  ...
  const getTypeWords = (cat: string) => {
    if (cat === 'CULTURE') {
      return ['history museum', 'local gallery', 'historic palace', 'ancient ruins', 'cultural festival', 'traditional workshop'];
    } else if (cat === 'NATURE') {
      return ['hidden lake', 'mountain vista', 'coastal trail', 'forest reserve', 'valley overlook', 'secret beach'];
    } else if (cat === 'FOOD') {
      return ['family-run bistro', 'street food stalls', 'cooking class', 'food festival', 'farmers market', 'wine vineyard'];
    } else if (cat === 'ADVENTURE') {
      return ['rock climbing spot', 'white water rapids', 'zip-line course', 'mountain biking trail', 'kayaking waters', 'paragliding launch'];
    } else if (cat === 'RELAXATION') {
      return ['thermal springs', 'beachfront yoga class', 'hillside meditation spot', 'traditional massage house', 'garden retreat', 'forest bath'];
    } else if (cat === 'SHOPPING') {
      return ['artisan market', 'antique district', 'local craft shops', 'designer boutiques', 'vintage stores', 'weekend bazaar'];
    } else if (cat === 'NIGHTLIFE') {
      return ['rooftop bar', 'jazz club', 'underground speakeasy', 'beachfront lounge', 'live music venue', 'local brewery'];
    } else if (cat === 'TRANSPORT') {
      return ['vintage tram', 'canal boat', 'scenic railway', 'coastal ferry', 'bicycle paths', 'vespa tour'];
    }
