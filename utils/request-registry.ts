...
class RequestRegistry {
  ...
  canMakeRequest(key: string, cooldownMs = 2000): boolean {
    ...
    if (entry?.retryCount >= this.maxRetries) {
      const backoffCooldown = this.calculateBackoff(entry.retryCount) * 1000;
...
export async function registeredFetch(
  ...
  try {
    ...
    if (response.status === 429) {
      ...
      console.warn(`[RequestRegistry] Rate limited by server for: ${requestKey}, retry after ${retryMs}ms`);
