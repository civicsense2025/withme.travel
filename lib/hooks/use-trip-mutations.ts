...
// Context interface for trip data provider
interface TripDataContext {
  tripData?: {
    trip: Trip | null;
    sections?: ItinerarySection[];
    items?: ItineraryItem[];
  };
  optimisticUpdate: (key: string, updateFn: (prev: any) => any) => Promise<void>;
  refetchTrip: () => Promise<void>;
  refetchItinerary: () => Promise<void>;
}

export function useTripMutations(tripId: string, tripContext: TripDataContext) {
  const { tripData, optimisticUpdate, refetchTrip, refetchItinerary } = tripContext;
  ...
  const deleteSection = async (sectionId: string): Promise<MutationResult<void>> => {
    ...
    try {
      // Find the section for optimistic update
      if (!tripData?.sections) {
        throw new Error('Trip sections data is not available');
      }
      ...
      // Also update items by removing those in this section
      ...
      // Actual API call
      const supabaseClient = getSupabase();
      const { error } = await supabaseClient.from('itinerary_sections').delete().eq('id', sectionId);
  ...
  const createItem = async (
    ...
    try {
      ...
      toast({
        title: 'Success',
        description: `Item "${item.title || 'New Item'}" added`,
      });
  ...
  const updateItem = async (
    ...
    try {
      ...
      if (!tripData?.items) {
        throw new Error('Trip items data is not available');
      }
