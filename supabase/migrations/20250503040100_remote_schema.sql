...
DROP FUNCTION IF EXISTS public.create_trip_with_owner(jsonb, uuid);
...
DROP FUNCTION IF EXISTS public.create_trip_with_owner(text, text, uuid, date, date, uuid, text, text, public.trip_type, public.privacy_setting);
...
DROP FUNCTION IF EXISTS public.create_trip_with_owner(text, uuid, text, text[], uuid, text, date, date, boolean, text, numeric, numeric);
...
-- We're explicitly using the first function signature to avoid ambiguity
CREATE POLICY "Allow members to view other members" ON "public"."trip_members" FOR SELECT USING ((EXISTS (
    SELECT 1
    FROM public.trip_members
