...
/** Props for the GroupCirclesSection component */
export interface GroupCirclesSectionProps {
  /** Theme mode override (useful for storybook or tests) */
  mode?: ThemeMode;
}
...
/**
 * GroupCirclesSection - An interactive showcase of group travel planning
 * 
 * @example
 * <GroupCirclesSection />
 * 
...
export function GroupCirclesSection({ mode }: GroupCirclesSectionProps) {
  ...
  const surfaceColor = useMemo(() => getColorToken('SURFACE', currentTheme), [currentTheme]);
  ...
  const getMemberPositionStyle = useCallback((index: number, total: number, groupSize: number): React.CSSProperties => {
    const memberAvatarCurrentSize = AVATAR_SIZE_STYLES[layout];

    if (isMobile) {
      // Simplified row for mobile avatars
      ...
      return {
        left: `${startX + index * (avatarWidth + spacing)}px`,
    ...
    <div style={sectionWrapperStyle}>
      ...
      <div style={ctaSectionStyle}>
        ...
        <p style={{ color: mutedColor, marginTop: '0.75rem', fontSize: '0.85rem' }}>
          Join thousands of travelers planning together
        </p>
      </div>
    </div>
  );
