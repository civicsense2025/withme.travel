...
export const TagInput: React.FC<TagInputProps> = ({ value, onChange, placeholder = 'Add a tag', existingTags = [] }) => {
  ...
  return (
    <div ref={containerRef} className="relative">
      <div className="flex flex-wrap gap-2 border border-input bg-background p-2 rounded-md min-h-[40px]">
        {value.map((tag, index) => (
          <div key={index} className="flex items-center bg-secondary text-secondary-foreground rounded px-2 py-0.5 text-sm">
            <span>{tag}</span>
            <button 
              type="button" 
