...
export function ItineraryTemplateDisplay({ template, sections }: ItineraryTemplateDisplayProps) {
  ...
  if (sections.length === 0) {
    ...
    sections.forEach((section) => {
      console.log(
        `[DEBUG] ItineraryTemplateDisplay - Section: ${section.id}, day: ${section.day_number}, items: ${section.items.length}`
      );
      if (section.items.length > 0) {
        console.log(
          `[DEBUG] ItineraryTemplateDisplay - Items for section ${section.id}: ${section.items.map((item) => item.title).join(', ')}`
        );
      }
    });
  }
