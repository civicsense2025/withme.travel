...
import { 
  Clock, Globe, Calendar, Lightbulb, DollarSign, 
  Heart, Accessibility, Leaf
} from 'lucide-react';
...
export const ItineraryMetadataSection: React.FC<ItineraryMetadataSectionProps> = ({ metadata }) => {
  ...
  if (!metadata || Object.keys(metadata).every(key => !metadata[key as keyof ItineraryMetadata])) {
    return null;
  }
              ...
              <div className="flex flex-col gap-1 mt-2">
                ...
                <div className="flex flex-wrap gap-2">
                  {avoid_seasons.map((season, i) => (
                    <Badge key={i} variant="outline" className="bg-background/80 text-muted-foreground">
                      {season}
                    </Badge>
                  ))}
                </div>
