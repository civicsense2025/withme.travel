...
import { ITINERARY_CATEGORIES, API_ROUTES } from '@/utils/constants';
...
export function ItineraryItemForm({
  ...
  const form = useForm<ItineraryFormValues>({
    ...
    defaultValues: {
      ...
      date: initialData?.date && typeof initialData.date === 'string' ? parseISO(initialData.date) : null,
  ...
  const handleLocationSelect = (place: MapboxPlace | null) => {
    if (place) {
      ...
      if (defaultCurrency && !form.getValues('currency')) {
         ...
         toast({ title: "Currency Updated", description: `Set to ${defaultCurrency} based on location (${place.country || place.countryCode}).`, duration: 2500 });
      }
  ...
  const onSubmit: SubmitHandler<ItineraryFormValues> = async (values) => {
    ...
    try {
      ...
      if (!response.ok) {
         ...
         throw new Error(errorData.error || `Failed to ${isEditing ? 'update' : 'add'} item: ${response.status}`);
      }
      ...
      const savedItem = savedItemResult.item || savedItemResult; 
       ...
       if (isEditing) {
          ...
          console.log("Calling onItemAdded with:", savedItem); // DEBUG
       ...
       toast({ title: 'Error', description: error.message || `Failed to ${isEditing ? 'save' : 'add'} item.`, variant: 'destructive' });
    }
  ...
  return (
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4 px-4 pt-4 pb-6 h-full flex flex-col overflow-hidden">
        <div className="flex-grow overflow-y-auto pr-3 space-y-4"> 
          <FormField control={form.control} name="title" render={({ field }) => (
              <FormItem> <FormLabel>Title*</FormLabel> <FormControl><Input placeholder="e.g., Dinner at Le Diplomate" {...field} disabled={isLoading} /></FormControl> <FormMessage /> </FormItem>
          )}/>
          ...
          <Collapsible open={showMoreOptions} onOpenChange={setShowMoreOptions}>
            <CollapsibleTrigger asChild>
               <Button variant="link" className="p-0 h-auto text-sm text-muted-foreground flex items-center">
                 {showMoreOptions ? <ChevronUp className="h-4 w-4 mr-1" /> : <ChevronDown className="h-4 w-4 mr-1" />}
            ...
            <CollapsibleContent className="space-y-4 pt-3">
              <FormField control={form.control} name="date" render={({ field }) => (
                <FormItem className="flex flex-col">
                  ...
                  <Popover>
                    ...
                    <PopoverContent className="w-auto p-0" align="start"> <Calendar mode="single" selected={field.value ?? undefined} onSelect={field.onChange} disabled={(date) => date < new Date("1900-01-01")} initialFocus /> </PopoverContent>
                  </Popover>
                  <FormMessage />
                </FormItem>
              )}/>
              <div className="grid grid-cols-2 gap-4">
                 <FormField control={form.control} name="start_time" render={({ field }) => (
                    <FormItem> <FormLabel>Start Time</FormLabel> <FormControl><Input type="time" {...field} value={field.value ?? ''} disabled={isLoading} /></FormControl> <FormMessage /> </FormItem>
                 )}/>
                 <FormField control={form.control} name="end_time" render={({ field }) => (
                    <FormItem> <FormLabel>End Time</FormLabel> <FormControl><Input type="time" {...field} value={field.value ?? ''} disabled={isLoading} /></FormControl> <FormMessage /> </FormItem>
                 )}/>
              </div>
               <FormField control={form.control} name="category" render={({ field }) => (
                  <FormItem> <FormLabel>Category</FormLabel> <Select onValueChange={field.onChange} defaultValue={field.value ?? undefined} disabled={isLoading}>
                      <FormControl><SelectTrigger><SelectValue placeholder="Select a category" /></SelectTrigger></FormControl>
                      <SelectContent>{Object.values(ITINERARY_CATEGORIES).map((cat) => (
              ...
              <div className="grid grid-cols-2 gap-4">
                   <FormField control={form.control} name="estimated_cost" render={({ field }) => (
                      <FormItem> <FormLabel>Est. Cost</FormLabel> <FormControl><Input type="number" placeholder="e.g., 50" min="0" step="1" {...field} value={field.value ?? ''} disabled={isLoading} /></FormControl> <FormMessage /> </FormItem>
                   )}/>
                   <FormField control={form.control} name="currency" render={({ field }) => (
                      <FormItem> <FormLabel>Currency</FormLabel> <FormControl><Input placeholder="e.g., USD" maxLength={3} {...field} value={field.value ?? ''} disabled={isLoading} /></FormControl> <FormMessage /> </FormItem>
                   )}/>
              </div>
