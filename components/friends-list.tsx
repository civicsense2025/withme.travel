...
export function FriendsList({
  ...
  if (variant === 'grid') {
    ...
    return (
      <div className={containerClassName || 'grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 gap-6 justify-center'}>
        {friends.map(friend => (
          <div key={friend.friend_id} className="flex flex-col items-center">
            <Avatar className="h-16 w-16 mb-2">
              <AvatarImage src={friend.friend_profile.avatar_url || ''} />
              <AvatarFallback>{friend.friend_profile.full_name.substring(0, 2).toUpperCase() || 'FR'}</AvatarFallback>
            </Avatar>
            <span className="text-xs text-center mt-1 text-muted-foreground">{friend.friend_profile.full_name}</span>
          </div>
        ))}
      </div>
    );
              ...
              <div 
                ...
                onClick={() => {
                  if (mode === 'select' && !isAlreadyAdded) {
                    setSelectedFriendIds(prev => prev.includes(friend.friend_id)
              ...
              >
                {mode === 'select' && !isAlreadyAdded && (
                  <Checkbox 
                    ...
                    className="mr-3"
                    onCheckedChange={() => setSelectedFriendIds(prev => prev.includes(friend.friend_id)
                      ? prev.filter(id => id !== friend.friend_id)
                      : [...prev, friend.friend_id])}
                  />
                )}
