...
export function CollaborativeNotes({
    ...
    async function fetchNoteDetails() {
      ...
      try {
        ...
        if (tagsResponse.ok) {
          ...
          const fetchedTags: Tag[] = (tagsData.tags || []).map((tag: any) => ({ id: tag.id, name: tag.name }))
  ...
  const saveTags = async (tagNamesToSave: string[], previousTags: Tag[]) => {
     ...
     try {
        ...
        const finalTags: Tag[] = (data.tags || []).map((tag: any) => ({ id: tag.id, name: tag.name }))
            ...
            {!readOnly && (
              <div>
                {isCreating ? (
                  ...
                  <Button size="sm" variant="ghost" onClick={() => setIsCreating(true)} className="gap-1">
            ...
            {isLoadingList ? (
              ...
              <ScrollArea className="h-[calc(100vh-20rem)]">
                <div className="divide-y divide-border">
                  {notesList.map((note) => (
                    <div key={note.id} className="flex items-center justify-between w-full text-left px-4 py-2 group">
                      <button
