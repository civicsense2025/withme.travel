...
// Comment out notification indicator import
  ...
  return (
    <header className="sticky top-0 z-40 w-full bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60 ">
      <div className="w-full flex h-16 items-center justify-between px-4 md:px-8 max-w-[2000px] mx-auto">
        <Link href="/" className="font-extrabold text-2xl md:text-2xl lg:text-3xl flex items-center tracking-tight leading-none select-none" style={{ letterSpacing: '-0.03em' }}>
          withme.
        </Link>
        ...
        <div className="hidden md:flex items-center space-x-4">
          ...
          <div className="relative group">
            ...
            <div className="absolute left-0 mt-2 w-48 rounded-xl shadow-lg bg-white dark:bg-black border border-border opacity-0 group-hover:opacity-100 group-focus-within:opacity-100 pointer-events-none group-hover:pointer-events-auto group-focus-within:pointer-events-auto transition-opacity z-50">
              <Link href="/trips/create" className="block px-5 py-3 text-sm hover:bg-travel-purple/10 rounded-t-xl transition-colors">Plan a Trip</Link>
              <Link href="/groups/create" className="block px-5 py-3 text-sm hover:bg-travel-purple/10 rounded-b-xl transition-colors">Form a Group</Link>
            </div>
          </div>
          <UserMenu />
        </div>
          ...
          <div className="relative">
            ...
            {mobileOpen && (
              <div className="absolute left-0 mt-2 w-48 rounded-xl shadow-lg bg-white dark:bg-black border border-border z-50">
                <Link href="/trips/create" className="block px-5 py-3 text-sm hover:bg-travel-purple/10 rounded-t-xl transition-colors" onClick={() => setMobileOpen(false)}>Plan a Trip</Link>
                <Link href="/groups/create" className="block px-5 py-3 text-sm hover:bg-travel-purple/10 rounded-b-xl transition-colors" onClick={() => setMobileOpen(false)}>Form a Group</Link>
              </div>
            )}
          </div>
          <Button
