      ...
      if (success) {
        setReactionsMap(prev => ({
          ...
          [commentId]: (prev[commentId] || [])
            .filter(r => !(r.user_id === 'currentUser' && r.emoji === emoji))
        }));
      }
    } catch (error) {
      ...
      Object.keys(repliesMap).forEach(parentId => {
        ...
        if (replies.some(r => r.id === commentId)) {
          setRepliesMap(prev => ({
            ...
            [`${parentId}_cache`]: prev[`${parentId}_cache`].map(r => r.id === commentId ? updated : r)
          }));
        }
      });
    }
    ...
    if (success) {
      ...
      Object.keys(repliesMap).forEach(parentId => {
        ...
        if (replies.some(r => r.id === commentId)) {
          setRepliesMap(prev => ({
            ...prev,
            [parentId]: prev[parentId].map(r => r.id === commentId ? { ...r, is_deleted: true, content: '[Deleted]' } : r),
            [`${parentId}_cache`]: prev[`${parentId}_cache`].map(r => r.id === commentId ? { ...r, is_deleted: true, content: '[Deleted]' } : r)
          }));
        }
      });
    }
    ...
    <div className={`flex flex-col w-full bg-transparent border-0 shadow-none ${className}`}>
      {showTitle && (
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-lg font-semibold">Comments <span className="font-normal text-muted-foreground">({totalComments})</span></h3>
          <Button
            variant="outline"
            size="sm"
            onClick={refresh}
      ...
      <div 
        className={`w-full mb-4 ${
          limitHeight ? 'max-h-[400px] overflow-y-auto pr-2' : ''
        }`}
      >
