...
function EmojiIcon({ className = '' }: { className?: string }) {
  return <span className={cn('inline-block', className)} role="img" aria-label="emoji">😊</span>;
}
    ...
    >
      ...
      <div className="flex items-center gap-2 mt-1">
        ...
        {onToggleReplies && comment.replies_count > 0 && (
          ...
          >
            {isLoadingReplies ? 'Loading...' : `${comment.replies_count} repl${comment.replies_count === 1 ? 'y' : 'ies'}`}
          </Button>
        )}
      </div>
      {showReactions && (
