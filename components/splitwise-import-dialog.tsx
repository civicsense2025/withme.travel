...
import { type SplitwiseUser } from "@/lib/services/splitwise"
...
export function SplitwiseImportDialog({ tripId, onOpenChange, isOpen, isLinked }: SplitwiseImportDialogProps) {
  ...
  const handleCheckboxChange = (candidate: ImportCandidate, checked: boolean) => {
    setSelectedUsers(prev => ({ ...prev, [`${candidate.status}-${candidate.id}`]: checked }))
  }
  ...
  const handleInvite = async () => {
    ...
    const usersToInvite: { email: string, role: string }[] = [];
    const usersToAdd: { userId: string, role: string }[] = [];
            ...
            >
              <div className="flex items-center space-x-3">
                <Checkbox 
                  ...
                  checked={selectedUsers[`${candidate.status}-${candidate.id}`] || false}
                  onCheckedChange={(checked) => handleCheckboxChange(candidate, !!checked)}
                  disabled={isInviting}
                />
      ...
      <DialogContent className="sm:max-w-[525px]">
        ...
        <DialogFooter className="mt-4">
          ...
          <Button 
            ...
            disabled={isLoading || isInviting || selectedCount === 0}
          >
            {isInviting ? (
