...
export function CitySearchInput({ 
  ...
  const searchCities = async (searchQuery: string, countryLimit = 20) => {
    ...
    try {
      const response = await fetch(`/api/cities/search?q=${encodeURIComponent(searchQuery)}&limit=10&countryLimit=${countryLimit}`);
      if (!response.ok) {
      ...
      setHasMoreResults(data.is_country_search === true && (data.cities || []).length >= countryLimit);
      setShowResults(true);
    } catch (err) {
