...
import { MapPin, Calendar as CalendarIcon, Users, Tag, Camera, Pencil, Share2, Lock, Link as LinkIcon, Globe } from "lucide-react"
...
function MemberAvatars({ members }: { members?: MemberWithProfile[] | null }) {
  ...
  return (
    <div className="flex items-center -space-x-1.5" title={`${members.length} member${members.length !== 1 ? 's' : ''}`}>
      {visibleMembers.map((member) => {
        ...
        return (
          <Avatar key={profile.id} className="inline-block h-6 w-6 rounded-full ring-1 ring-background" title={name}>
...
export function TripHeader({ 
  ...
  const [editingDates, setEditingDates] = React.useState<{ start: string | null; end: string | null }>({ start: startDate ?? null, end: endDate ?? null });
  ...
  return (
    <div>
      {/* Cover Image Section */}
      <div className="relative w-full h-48 md:h-64 mb-6 group"> {/* Standardized height and bottom margin */}
        <div className="relative w-full h-full rounded-lg overflow-hidden shadow-sm"> 
          <Image
            src={coverImageUrl || '/images/destinations/default-placeholder.jpg'}
      ...
      {/* Main Info Block */}
      <div className="px-1"> {/* Optional: Add slight horizontal padding if needed */}
        {/* Title */} 
        ...
        <div className="flex flex-wrap items-center gap-x-4 gap-y-2 text-sm text-muted-foreground mb-4"> 
           ...
           {tags && tags.length > 0 && (
            <div className="flex flex-wrap items-center gap-1.5">
              {tags.map((tag) => (
                <Badge key={tag.id} variant="secondary" className="font-normal text-xs px-1.5 py-0.5"> 
                  {tag.name}
                </Badge>
              ))}
            </div>
          ...
          {destinationId && destinationName && (
             <Link href={`/destinations/${destinationId}`} className="flex items-center hover:text-foreground transition-colors">
              ...
              <TooltipProvider>
                <Tooltip>
                  <TooltipTrigger asChild>
                    <Button variant="outline" size="sm" className="px-2 py-1 h-auto border-dashed border-2" onClick={() => setDatePopoverOpen(true)}>
          ...
          {members && members.length > 0 && (
            <span className="flex items-center cursor-pointer" onClick={onMembers} title="View Members">
