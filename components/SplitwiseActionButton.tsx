...
import { SplitwiseGroup } from "@/lib/services/splitwise" // Assuming type path

interface SplitwiseActionButtonProps {
  tripId: string
  isLinked: boolean | null // null means status is being checked
  isConnected: boolean | null // null means status is being checked
  isLoading: boolean // True during connect/link/unlink actions
  groups: SplitwiseGroup[] | null
  error: string | null
  linkedGroupName?: string | null // Add optional prop for linked group name
  onConnect: () => void
  onLink: (groupId: string) => void
  onUnlink: () => void
  onDisconnect: () => void // Add new prop for disconnect handler
}

export function SplitwiseActionButton({
  tripId,
  isLinked,
  isConnected,
  isLoading,
  groups,
  error,
  linkedGroupName, // Destructure the new prop
  onConnect,
  onLink,
  onUnlink,
  onDisconnect, // Destructure new prop
}: SplitwiseActionButtonProps) {
  ...
  const handleGroupSelectAndLink = (groupId: string) => {
    onLink(groupId)
            ...
            {(groups && groups.length > 0) ? (
              <DropdownMenuSub>
                 ...
                 <DropdownMenuPortal>
                  <DropdownMenuSubContent>
                     {groups.map((group) => (
                        <DropdownMenuItem key={group.id} onClick={() => handleGroupSelectAndLink(group.id.toString())}>
                          {group.name}
                        </DropdownMenuItem>
                      ))}
                  </DropdownMenuSubContent>
