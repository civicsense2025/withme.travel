...
import { ThumbsUp, ThumbsDown, MessageCircle, Calendar, MapPin, DollarSign, HelpCircle, Clipboard, Bookmark, Heart, Send } from 'lucide-react';
...
export function IdeaCard({
  ...
  const fetchCommentCount = async () => {
    try {
      const response = await fetch(`/api/group-idea-comments/count?ideaId=${idea.id}`);
  ...
  const fetchComments = async () => {
    try {
      ...
      const response = await fetch(`/api/group-idea-comments?ideaId=${idea.id}&limit=50&offset=0`);
  ...
  const handleSubmitComment = async (event?: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch('/api/group-idea-comments', {
        method: 'POST',
        headers: {
  ...
  const getGroupedReactions = () => {
    ...
    const groupedReactions: Record<string, Array<{
      id: string;
      name: string;
    }>> = {};
              ...
              <TooltipContent>
                <div className="py-1">
                  ...
                  <ul className="space-y-1">
                    {users.map(user => (
                      <li key={user.id} className="text-xs">{user.name}</li>
                    ))}
                  </ul>
                </div>
              </TooltipContent>
  ...
  const renderCompactReactions = () => {
    const groupedReactions = getGroupedReactions();
    const totalReactions = Object.values(groupedReactions).reduce((sum, users) => sum + users.length, 0);
                ...
                {Object.entries(groupedReactions).map(([type, users]) => (
                  <div key={type} className="pb-2 border-b last:border-0">
                    <div className="flex items-center mb-1">
                      {type === 'heart' ? <Heart className="h-4 w-4 mr-1 text-red-500" /> : <span className="mr-1">{type}</span>}
                    ...
                    {comments.map((comment) => (
                      <div key={comment.id} className="flex gap-3 pb-3 border-b">
                        <Avatar className="h-8 w-8">
                          ...
                          />
                          <AvatarFallback>
                            {(comment.user?.user_metadata?.full_name || comment.user?.email || 'U').charAt(0)}
                          </AvatarFallback>
                        </Avatar>
                        <div className="flex-1">
                          <div className="flex justify-between">
                            <p className="text-sm font-medium">
                              {comment.user?.user_metadata?.full_name || comment.user?.email || 'User'}
                            </p>
