...
export function TagList({ destinationId, className }: TagListProps) {
  ...
  useEffect(() => {
    const loadTags = async () => {
      ...
      const tags: Tag[] = (data || []).map((dt: any) => ({
        ...dt.tag,
        category: dt.tag?.category ?? '',
        created_at: dt.tag?.created_at ?? '',
        created_by: dt.tag?.created_by ?? '',
        description: dt.tag?.description ?? '',
        emoji: dt.tag?.emoji ?? '',
        id: dt.tag?.id ?? '',
        name: dt.tag?.name ?? '',
        slug: dt.tag?.slug ?? '',
        updated_at: dt.tag?.updated_at ?? '',
        use_count: dt.tag?.use_count ?? 0,
        is_verified: dt.tag?.is_verified ?? false,
      }));
      setTags(tags);
    };
    loadTags();
