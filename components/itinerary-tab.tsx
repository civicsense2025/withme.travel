...
export function ItineraryTab({ tripId, canEdit = false }: ItineraryTabProps) {
          ...
          if (item.id === itemId) {
            if (isUpvote) {
              ...
              if (item.user_vote === "up") {
                return { ...item, votes: (item.votes || 0) - 1, user_vote: null }
              }
              // If downvoted, remove downvote and add upvote
              else if (item.user_vote === "down") {
                return { ...item, votes: (item.votes || 0) + 2, user_vote: "up" }
              }
              // If no vote, add upvote
              else {
                return { ...item, votes: (item.votes || 0) + 1, user_vote: "up" }
              }
            } else {
              ...
              // If upvoted, remove upvote and add downvote
                ...
                return { ...item, votes: (item.votes || 0) - 2, user_vote: "down" }
              }
              // If no vote, add downvote
              else {
                return { ...item, votes: (item.votes || 0) - 1, user_vote: "down" }
              }
            }
          }
          return item
  ...
  // Group items by date
  const itemsByDate = itineraryItems.reduce(
    (acc, item) => {
