...
interface ViatorExperienceGridProps {
  title: string;
  subtitle?: string;
  experiences: ViatorExperienceProps[];
  showViewAll?: boolean;
  viewAllUrl?: string;
  tripId?: string;
  categories?: string[];
...
export function ViatorExperienceGrid({
  ...
  const filteredExperiences = experiences.filter(exp => {
    ...
    if (selectedCategory !== 'all' && 
        !exp.labels?.includes(selectedCategory)) {
      return false;
    }
    ...
    if (durationFilter && exp.duration) {
      if (durationFilter === 'short' && exp.duration.includes('hour') && 
    ...
    <div className="mb-10 w-full">
      <div className="mb-6 flex items-center justify-between">
        <div>
          ...
          {subtitle && (
            <p className="mt-1 text-secondary-text">{subtitle}</p>
          )}
        </div>
