    ...
    <div className={`relative ${outlineClass} ${className}`}>
      ...
      <div className="absolute top-0 right-0 flex space-x-1 bg-white/90 border border-gray-200 rounded shadow-sm p-1 z-50">
        <button
          ...
          className={`p-1 rounded-sm hover:bg-gray-100 ${showOutline ? 'bg-accent' : ''}`}
        ...
        <button
          ...
          className={`p-1 rounded-sm hover:bg-gray-100 ${showGrid ? 'bg-accent' : ''}`}
          title="Toggle grid"
        >
      ...
      {showGrid && (
        <div className="absolute inset-0 grid grid-cols-12 gap-4 pointer-events-none z-40">
          {Array.from({ length: 12 }).map((_, i) => (
            <div key={i} className="h-full bg-accent bg-opacity-10 border-x border-border" />
          ))}
        </div>
      )}
    </div>
