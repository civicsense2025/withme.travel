...
export function AuthDebugger() {
  ...
  const [cookieInfo, setCookieInfo] = useState<{[key: string]: string}>({})
  ...
  const addError = (context: string, error: string, type: "error" | "info" = "error") => {
    setErrors(prev => [{
        ...
        <div className="space-y-2">
          ...
          <ScrollArea className="h-[200px] w-full rounded-md border">
            <div className="p-4 space-y-3">
              {errors.map((error, i) => (
                <div key={i} className="text-sm space-y-1">
                  <div className="flex items-center gap-2">
                    <Badge variant={error.context?.includes("info") ? "outline" : "destructive"} className="h-5">
              ...
              {errors.length === 0 && (
                <p className="text-muted-foreground text-sm">No auth events recorded yet</p>
              )}
            </div>
          </ScrollArea>
        </div>
