...
export function TripSidebarContent({ // Export the component
  description,
  privacySetting,
  startDate,
  endDate,
    ...
    <Card>
      ...
      <CardContent className="space-y-4">
        <div>
          ...
          <p className="text-sm">{description || "No description provided."}</p> {/* Added default text */}
        </div>
        <div>
          <Label className="text-xs font-medium text-muted-foreground">Privacy</Label>
          <p className="text-sm capitalize">{privacySetting?.replace('_', ' ') || 'Private'}</p> {/* Improved display */}
        </div>
        <div>
          <Label className="text-xs font-medium text-muted-foreground">Dates</Label>
          <p className="text-sm">{startDate && endDate ? formatDateRange(startDate, endDate) : "Dates not set"}</p> {/* Added default text */}
        </div>
        <div>
        ...
        {canEdit && (
          <Button variant="outline" size="sm" className="mt-4 w-full" onClick={onEdit}> {/* Added margin-top */}
