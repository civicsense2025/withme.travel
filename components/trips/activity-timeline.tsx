...
import { useActivityTimeline, ActivityTimelineItem } from '@/hooks/use-activity-timeline';
...
// Extend ActionType to include trip history action types and all used action types
// (add missing types from switch/case and usages)
  ...
  const getActivityIcon = (actionType: ExtendedActionType) => {
    switch (actionType) {
      case 'ITINERARY_ITEM_ADDED':
        return <Plus className="h-4 w-4 text-green-500" aria-label="Item added" />;
      case 'ITINERARY_ITEM_UPDATED':
        return <Edit className="h-4 w-4 text-orange-500" aria-label="Item updated" />;
      case 'ITINERARY_ITEM_DELETED':
        return <X className="h-4 w-4 text-red-500" aria-label="Item deleted" />;
      case 'MEMBER_ADDED':
        return <Users className="h-4 w-4 text-blue-500" aria-label="Member added" />;
      case 'MEMBER_REMOVED':
        return <User className="h-4 w-4 text-red-500" aria-label="Member removed" />;
      case 'MEMBER_ROLE_UPDATED':
        return <User className="h-4 w-4 text-blue-500" aria-label="Member role updated" />;
      case 'COMMENT_ADDED':
      case 'COMMENT_UPDATED':
      case 'COMMENT_DELETED':
        return <MessageCircle className="h-4 w-4 text-blue-500" aria-label="Comment activity" />;
      case 'VOTE_CAST':
        return <Vote className="h-4 w-4 text-indigo-500" aria-label="Vote cast" />;
      case 'ACCESS_REQUEST_UPDATED':
        return <Check className="h-4 w-4 text-green-500" aria-label="Access request updated" />;
      case 'IMAGE_UPLOADED':
        return <ImageIcon className="h-4 w-4 text-blue-500" aria-label="Image uploaded" />;
      case 'TAG_ADDED':
      case 'TAG_REMOVED':
        return <Tag className="h-4 w-4 text-yellow-500" aria-label="Tag modified" />;
      case 'NOTE_CREATED':
      case 'NOTE_UPDATED':
      case 'NOTE_DELETED':
        return <FileText className="h-4 w-4 text-orange-500" aria-label="Note activity" />;
      case 'FOCUS_INITIATED':
        return <Target className="h-4 w-4 text-red-500" aria-label="Focus initiated" />;
      // Trip history specific icons
      case 'TRIP_VISITED':
      case 'TRAVEL_HISTORY_ADDED':
        return <History className="h-4 w-4 text-green-500" aria-label="Trip history" />;
      case 'TRIP_PLANNED':
        return <Calendar className="h-4 w-4 text-blue-500" aria-label="Trip planned" />;
      case 'DESTINATION_VISITED':
        return <Map className="h-4 w-4 text-purple-500" aria-label="Destination visited" />;
      case 'DESTINATION_RATED':
        return <Bookmark className="h-4 w-4 text-yellow-500" aria-label="Destination rated" />;
      default:
        return <Clock className="h-4 w-4 text-muted-foreground" aria-label="Activity" />;
    }
  };
      ...
      <div key={historyItem.id} className="relative pl-10 pr-4 py-3 hover:bg-muted/30">
        <div className="absolute left-4 top-4 flex items-center justify-center z-10">
          <div className="h-4 w-4 rounded-full bg-background border-2 border-muted flex items-center justify-center">
            <Map className="h-4 w-4 text-blue-500" aria-label="Trip location" />
          </div>
        </div>
