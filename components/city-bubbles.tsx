  ...
  return (
    ...
    >
      <span className="text-sm font-medium text-black/70 dark:text-white/80 flex items-center gap-2">
        {isBubbleLoading ? (
          <>
            <svg className="animate-spin h-4 w-4 text-accent-purple" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
...
export function CityBubbles() {
  ...
  const handleBubbleClick = async (cityName: string) => {
    ...
    try {
      ...
      if (!response.ok) {
        console.warn('[CityBubbles] Server returned error but checking for valid trip ID:', responseData);
        
        // If we still received a tripId despite the error, we can use it
        if (responseData && responseData.tripId) {
          console.log('[CityBubbles] Found valid tripId despite error, proceeding:', responseData.tripId);
