...
export function Task({
  ...
  const handleVote = async (itemId: string, voteType: 'up' | 'down') => {
    ...
    if (!currentVote) {
      ...
      if (voteType === 'up') {
        newVotes.up += 1;
        newVotes.upVoters = [...newVotes.upVoters, { id: 'current-user', name: 'You', avatar_url: null, username: null }];
      } else {
        newVotes.down += 1;
        newVotes.downVoters = [...newVotes.downVoters, { id: 'current-user', name: 'You', avatar_url: null, username: null }];
      }
  ...
  const getPriorityBadge = (priority?: string | null) => {
    ...
    switch (priority) {
      ...
      case 'medium':
        return (
          <Badge variant="outline" className="ml-2 border-amber-600/40 bg-amber-500/10 text-amber-700 dark:text-amber-400">
            Medium
          </Badge>
        );
      case 'low':
        return (
          <Badge variant="outline" className="ml-2 border-green-600/40 bg-green-500/10 text-green-700 dark:text-green-400">
            Low
          </Badge>
        );
      default:
        ...
        <Card key={item.id} className="group hover:shadow-sm transition-shadow duration-200">
          <CardHeader className="pb-2">
            <div className="flex justify-between items-start">
              <div>
                <CardTitle className="group-hover:text-primary transition-colors duration-200">{item.title}</CardTitle>
