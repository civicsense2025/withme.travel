    ...
    <div className="border-b border-border/40 dark:border-border/20 py-4 last:border-0">
      ...
      >
        <div className="flex items-center">
          <span className="text-2xl mr-3 flex items-center justify-center w-8" role="img" aria-hidden="true">
            {emoji}
          </span>
          <h2 className="text-xl font-medium">{title}</h2>
        </div>
        <ChevronDown
          className={cn("h-5 w-5 text-muted-foreground transition-transform duration-300", 
            isOpen && "rotate-180"
          )}
        />
      </button>
...
export default function DestinationDetails({ destination, tags, metadata = {} }: DestinationDetailsProps) {
  ...
  const bestTimesToVisit = metadata.best_times_to_visit || ["Spring (April-June)", "Fall (September-October)"];
  const timesToAvoid = metadata.times_to_avoid || ["August (many local businesses close)", "Mid-winter (January-February)"];
  const accessibility = metadata.accessibility || "Moderate - extensive metro system but many historic sites have stairs; cobblestone streets in historic areas";
  ...
  const displayTags = tags?.length ? tags : ["Art lovers", "Romantics", "Food enthusiasts", "Architecture admirers"];
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2 mt-3">
                {languageBadges.map((lang, idx) => (
                  <span key={idx} className="px-4 py-1.5 bg-secondary text-secondary-foreground rounded-full text-sm">
                    {lang}
                  </span>
                ))}
              </div>
              ...
              <div className="mt-3">
                ...
                <div className="flex flex-wrap gap-2 mt-2">
                  {bestTimesToVisit.map((time, index) => (
                    <span key={index} className="px-4 py-1.5 bg-green-50 text-green-700 dark:bg-green-950/50 dark:text-green-300 rounded-full text-sm">
                      {time}
                    </span>
                  ))}
                </div>
                ...
                <div className="flex flex-wrap gap-2 mt-2">
                  {timesToAvoid.map((time, index) => (
                    <span key={index} className="px-4 py-1.5 bg-muted/50 text-muted-foreground rounded-full text-sm">
                      {time}
                    </span>
                  ))}
                </div>
