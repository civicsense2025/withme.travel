...
export function PopularDestinations({ onSelect }: PopularDestinationsProps) {
  ...
  if (loading) return <div className="p-4 text-center text-muted-foreground">Loading popular destinations…</div>;
        ...
        {destinations.map(dest => (
          ...
          >
            <div className="flex flex-col items-center p-4">
              ...
              {dest.byline && <div className="text-xs text-muted-foreground text-center mt-1">{dest.byline}</div>}
            </div>
          </Card>
        ))}
      </div>
