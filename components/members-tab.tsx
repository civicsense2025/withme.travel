...
export function MembersTab({ tripId, canEdit = false, userRole = null, initialMembers = [] }: MembersTabProps) {
  ...
  const handleAddMember = async () => {
    if (!newMember.email || !newMember.role) {
      toast({ title: "Missing fields", description: "Please provide email and role.", variant: "destructive" });
      return;
    }
    ...
    try {
      ...
      toast({ title: "Invitation Error", description: error.message || "Failed to send invitation", variant: "destructive" });
    }
  };

  const handleRemoveMember = async (memberToRemoveId: string) => {
    if (memberToRemoveId === 'YOUR_USER_ID') { // Replace with actual check for current user
  ...
  const handleAccessRequest = async (requestId: string, approve: boolean) => {
    try {
      const response = await fetch(`${API_ROUTES.PERMISSION_REQUESTS(tripId)}/${requestId}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ status: approve ? PERMISSION_STATUSES.APPROVED : PERMISSION_STATUSES.REJECTED }),
      });
      ...
      toast({ title: "Error", description: error.message || "Failed to process access request", variant: "destructive" });
    }
  };
            ...
            {members.map(member => (
              <li key={member.user_id} className="flex items-center justify-between">
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    <p className="font-medium">{member.profiles?.name ?? 'User Profile Not Found'}</p>
        ...
        {accessRequests.length > 0 ? (
          <ul className="space-y-4">
            {accessRequests.map(req => (
              <li key={req.id} className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 border p-3 rounded-md">
                ...
                <div className="flex gap-2 self-end sm:self-center">
                  <Button variant="outline" size="icon" onClick={() => handleAccessRequest(req.id, false)} className="text-destructive hover:bg-destructive/10 border-destructive/50">
                    <X className="h-4 w-4" />
                  </Button>
                  <Button variant="outline" size="icon" onClick={() => handleAccessRequest(req.id, true)} className="text-green-600 hover:bg-green-600/10 border-green-600/50">
          ...
          <p className="text-muted-foreground italic text-center py-4">No pending access requests.</p>
        )}
      </CardContent>
    </Card>
  );
  ...
  const InviteLinkCard = () => (
    <Card>
      <CardHeader>
        ...
        <CardDescription>Anyone with this link can view the trip (if public) or request access.</CardDescription>
      </CardHeader>
      <CardContent className="flex items-center gap-2">
        <Input value={inviteLink} readOnly className="flex-1 bg-muted" />
        <Button variant="outline" size="icon" onClick={copyToClipboard} aria-label="Copy invite link">
        ...
        <DialogContent>
          ...
          <div className="grid gap-4 py-4">
            <div className="grid grid-cols-4 items-center gap-4">
              ...
              <Input id="email" type="email" value={newMember.email} onChange={(e) => setNewMember({ ...newMember, email: e.target.value })} className="col-span-3" placeholder="member@example.com" />
            </div>
            <div className="grid grid-cols-4 items-center gap-4">
              <Label htmlFor="role" className="text-right">Role</Label>
              <Select value={newMember.role} onValueChange={(value) => setNewMember({ ...newMember, role: value as TripRole })}> 
