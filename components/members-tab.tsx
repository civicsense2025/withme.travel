...
import { Mail, PlusCircle, Trash2, User, Check, X, UserPlus, Link2, Link2Off, RefreshCw, Loader2, ChevronDown, Import } from "lucide-react"
...
interface MembersTabProps {
  ...
  initialSplitwiseGroupId?: number | null // Pass initial linked group ID from server
  isSplitwiseConnected?: boolean | null // Add prop to know if the *account* is connected
  linkedSplitwiseGroupIdFromParent?: number | null // Add prop for the current linked group ID
}
...
export function MembersTab({ tripId, canEdit = false, userRole = null, initialMembers = [], initialSplitwiseGroupId = null, isSplitwiseConnected = null, linkedSplitwiseGroupIdFromParent = null }: MembersTabProps) {
  ...
  const [linkedSplitwiseGroupId, setLinkedSplitwiseGroupId] = useState<number | null>(initialSplitwiseGroupId); // Store the ID of the linked group for THIS trip
  const [availableSplitwiseGroups, setAvailableSplitwiseGroups] = useState<SplitwiseGroup[]>([]);
  const [selectedSplitwiseGroupId, setSelectedSplitwiseGroupId] = useState<string>(""); // For the Select dropdown
  ...
  const handleUnlinkGroup = async () => {
     ...
     try {
       ...
       const response = await fetch(API_ROUTES.TRIP_DETAILS(tripId), { // Using trip details PATCH for now
         method: 'PATCH',
         headers: { 'Content-Type': 'application/json' },
  ...
  const memberCards = members && members.length > 0 ? (
    members.map((member) => {
      // Add safe defaults for potentially null profile data
      const memberName = member.profiles?.name || "Invited User";
      const memberRole = member.role?.toLowerCase() || TRIP_ROLES.VIEWER.toLowerCase();
      ...
      return (
        <Card key={member.id} className="flex items-center justify-between p-4">
          ...
          <div className="flex items-center gap-2">
            ...
            {canEdit && ![TRIP_ROLES.ADMIN.toLowerCase()].includes(memberRole) && (
              <Button
                variant="ghost"
                size="icon"
                onClick={() => handleRemoveMember(member.id)} // Pass the correct member ID
  ...
  return (
    <div className="space-y-6 py-4">
      ...
      <SplitwiseImportDialog
        ...
        tripId={tripId}
        isLinked={linkedSplitwiseGroupIdFromParent !== null}
      />
    </div>
  )
}
