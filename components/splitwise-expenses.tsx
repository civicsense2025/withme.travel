...
import { DollarSign, RefreshCw, ExternalLink, Clock } from "lucide-react"
...
// Update props to receive state and fetch function from parent
interface SplitwiseExpensesProps {
...
export function SplitwiseExpenses({
  expenses,
  isLoading,
  error,
  groupId,
  lastUpdated,
  onRefresh
}: SplitwiseExpensesProps) {

  // Remove internal state management (useState, useEffect, fetchExpenses)

  const renderCardFooter = () => (
    <CardFooter className="text-xs text-muted-foreground justify-between items-center pt-4 border-t">
