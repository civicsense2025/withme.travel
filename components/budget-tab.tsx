...
export function BudgetTab({
  ...
  const memberExpenseSummary = useMemo(() => {
    const summary = members.map(member => {
      ...
      const share = manualExpenses.reduce((sum, exp) => sum + (Number(exp.amount) / memberCount), 0);
      ...
      <Card className="shadow-sm border-muted/50">
        <CardHeader className="flex flex-row items-start justify-between pb-2">
          ...
          <div className="text-right bg-muted/20 px-4 py-2 rounded-xl">
            ...
            <div className="text-2xl font-bold">{formatCurrency(totalManualSpent)}/{formatCurrency(budgetTotal)}</div>
          </div>
        </CardHeader>
        <CardContent className="space-y-6 pt-4">
          <div className="flex flex-wrap gap-4 border-b pb-4">
            <Badge variant="secondary" className="text-sm px-4 py-1.5 rounded-full flex items-center gap-2 bg-surface-light dark:bg-surface-light/10">
                  ...
                  <ScrollArea className="max-h-[300px]">
                    <div className="p-4 space-y-3">
                      {memberExpenseSummary.map((member) => (
                        <div key={member.id} className="flex items-center justify-between p-3 border-b border-dashed last:border-0 rounded-lg hover:bg-surface-light/50 dark:hover:bg-surface-light/5 transition-colors">
                ...
                <CardContent className="p-5 pt-5">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="space-y-1.5">
                      <Label htmlFor="title" className="text-xs font-medium">Description</Label>
                      <Input
                        id="title"
                    ...
                    <div className="space-y-1.5">
                      <Label htmlFor="paid_by" className="text-xs font-medium">Paid By</Label>
                      <Select
                ...
                <AccordionContent>
                  <div className="space-y-4 py-2">
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground">By default, expenses are split equally among all trip members. The member summary shows who has paid what and who owes money to whom.</p>
                    </div>
                    <div>
                      <h4 className="font-medium">How do I add an expense?</h4>
                      <p className="text-sm text-muted-foreground">Click the "Add Expense" button and fill out the form with the expense details, including who paid and the date of the expense.</p>
                    </div>
                    <div>
                      <h4 className="font-medium">Can I set a budget limit?</h4>
                      <p className="text-sm text-muted-foreground">Yes, you can set a budget target by clicking the pencil icon next to the total at the top right of this section.</p>
                    </div>
                    <div>
                      <h4 className="font-medium">How do I export expenses?</h4>
                      <p className="text-sm text-muted-foreground">Currently, you can screenshot this page for your records. We're working on adding CSV export functionality soon.</p>
                    </div>
                  </div>
                </AccordionContent>
              </AccordionItem>
