...
if (!UNSPLASH_ACCESS_KEY) {
  console.error(chalk.red('Error: UNSPLASH_ACCESS_KEY not found. Add it to .env.local for the CLI to function.'));
  process.exit(1);
}
 ...
 */
const downloadImage = async (photo: UnsplashPhoto, filename: string, outputDir: string): Promise<string> => {
  ...
  try {
    ...
    await new Promise((resolve, reject) => {
      // Assuming response.body is Node.js stream-like
 ...
 */
const withRetry = async <T>(fn: () => Promise<T>, retries = DEFAULT_RETRY_COUNT, delay = DEFAULT_RETRY_DELAY): Promise<T> => {
  ...
  for (let i = 0; i <= retries; i++) {
    try {
      ...
      if (i < retries) {
        console.log(chalk.yellow(`Operation failed (attempt ${i + 1}/${retries + 1}). Retrying in ${delay / 1000}s... Error: ${lastError.message}`));
...
const createAttributionFile = (photo: UnsplashPhoto, outputPath: string, format: 'text'|'html'|'json' = 'text'): void => {
  ...
  try {
    ...
    console.error(chalk.red(`Error saving attribution file ${attributionPath}: ${(error as Error).message}`));
  }
};
...
program
  ...
  .option('-f, --filename <name>', 'Custom filename (without extension). Default: generated from city/country')
  .option('--attribution <format>', 'Attribution file format (text, html, json)', 'text')
  .option('--rate-limit <number>', `Max requests per hour (Service default: 50) - CLI option currently informational`) // Informational only
  .option('--force-country', 'Force using country query if city/state yields no results (use with caution)', false)
  .option('--retries <number>', `Number of retries on API/download failure (default: ${DEFAULT_RETRY_COUNT})`, DEFAULT_RETRY_COUNT.toString())
  .action(async (options: DownloadCityOptions) => {
    ...
    try {
        ...
        if (isNaN(retryCount) || retryCount < 0) {
           spinner.warn({text: `Invalid retry count "${options.retries}". Using default: ${DEFAULT_RETRY_COUNT}`});
           options.retries = DEFAULT_RETRY_COUNT.toString();
        }

        // Use getDestinationPhoto imported from the service
        const { photo, attribution, sourceQuery } = await withRetry(
        ...
        const filenameExt = path.extname(new URL(photo.urls.raw || photo.urls.full || photo.urls.regular).pathname) || '.jpg';
        ...
        spinner.error({ text: `Failed to download image for ${options.city}: ${(error as Error).message}` });
        process.exitCode = 1;
      }
  });
...
program
  ...
  .option('--attribution <format>', 'Attribution file format (text, html, json) for each image', 'text')
  .option('--rate-limit <number>', `Max requests per hour (Service default: 50) - CLI option currently informational`)
  .option('--batch-size <number>', `Number of parallel downloads (default: ${DEFAULT_BATCH_SIZE})`, DEFAULT_BATCH_SIZE.toString())
      ...
      try {
          ...
          const batchSize = parseInt(options.batchSize, 10);
           if (isNaN(batchSize) || batchSize <= 0) {
              batchSpinner.warn({text: `Invalid batch size "${options.batchSize}". Using default: ${DEFAULT_BATCH_SIZE}`});
              options.batchSize = DEFAULT_BATCH_SIZE.toString();
           }
          const retryCount = parseInt(options.retries, 10);
           if (isNaN(retryCount) || retryCount < 0) {
              batchSpinner.warn({text: `Invalid retry count "${options.retries}". Using default: ${DEFAULT_RETRY_COUNT}`});
              options.retries = DEFAULT_RETRY_COUNT.toString();
           }
          ...
          const lines = csvContent.split('\n').map(l => l.trim()).filter(l => l); // Split, trim, remove empty lines
          ...
          const locations = lines.slice(startIndex).map((line, index) => {
              const parts = line.split(options.delimiter).map(p => p.trim());
              if (parts.length < 2) {
                  batchSpinner.warn({ text: `Skipping invalid line ${index + startIndex + 1}: Not enough columns (expected city, country, [state]). Line: "${line}"` });
                  return null;
              }
              return {
                  city: parts[0],
          ...
          for (let i = 0; i < locations.length; i += batchSize) {
              ...
              batchSpinner.update({ text: `Processing batch ${Math.floor(i / batchSize) + 1}/${Math.ceil(locations.length / batchSize)} ...` });

              const promises = batch.map(async (location) => {
                  if (!location) return;
                  const { city, country, state, originalLine } = location;
                  const locationSpinner = createSpinner(`Processing: ${city}, ${country}... (Line ${originalLine})`).start();

                  try {
                      // Use getDestinationPhoto from service
                      const { photo, attribution, sourceQuery } = await withRetry(
                          () => getDestinationPhoto(city, country, state),
                          retryCount
                      );
          ...
          batchSpinner.success({ text: `Batch download complete. Success: ${successCount}, Errors: ${errorCount}` });
          if (errors.length > 0) {
...
program
  ...
  .option('--attribution <format>', 'Create attribution file(s) if downloading (text, html, json)', undefined)
  .option('--rate-limit <number>', `Max requests per hour (Service default: 50) - CLI option currently informational`)
  .option('--retries <number>', `Number of retries on API failure (default: ${DEFAULT_RETRY_COUNT})`, DEFAULT_RETRY_COUNT.toString())
  .action(async (options: SearchOptions) => {
    ...
    try {
        ...
        if (isNaN(perPage) || perPage <= 0 || perPage > 30) throw new Error('Invalid per-page value (must be 1-30).');
        const retryCount = parseInt(options.retries, 10);
         if (isNaN(retryCount) || retryCount < 0) {
             searchSpinner.warn({text: `Invalid retry count "${options.retries}". Using default: ${DEFAULT_RETRY_COUNT}`});
             options.retries = DEFAULT_RETRY_COUNT.toString();
         }
...
if (process.env.NODE_ENV !== 'test') {
    if (process.argv.length > 2) {
      ...
      if (process.argv[1] && (scriptName.includes('unsplash') || scriptName === 'index.js' || scriptName === 'cli.js')) {
