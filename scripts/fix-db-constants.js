...
function processFile(filePath, isPriority = false) {
  ...
  try {
    ...
    if (!content.includes('@/utils/constants/database') && 
    ...
    if (!content.includes('const ENUMS =')) { // Don't replace if defining a local ENUMS constant
      content = content.replace(/ENUMS\./g, 'DB_ENUMS.');
    }
    
    if (!content.includes('const FIELDS =')) { // Don't replace if defining a local FIELDS constant
