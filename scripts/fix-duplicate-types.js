...
async function fixImportsInFile(filePath) {
  ...
  for (const typeName of DATABASE_TYPES) {
    ...
    const otherImportRegex = new RegExp(
      `import\\s+(?:{[^}]*?(?:\\b${typeName}\\b)[^}]*?}|\\b${typeName}\\b)\\s+from\\s+['"](?!@/utils/constants/database)[^'"]*['"]`,
      'g'
    );
    const hasOtherImport = otherImportRegex.test(updated);

    // Look for import from database constants
    const dbImportRegex = new RegExp(
      `import\\s+{[^}]*?(?:\\b${typeName}\\b)[^}]*?}\\s+from\\s+['"]@/utils/constants/database['"]`,
      'g'
    );
  ...
  const typeAssertionRegex =
    /import\s+{\s*(?:type\s+)?(\w+)(?:\s*,\s*(?:type\s+)?(\w+))*\s*}\s+from\s+['"]@\/utils\/constants\/database['"]/g;
...
async function main() {
  const tsFiles = await glob('**/*.{ts,tsx}', {
    cwd: ROOT_DIR,
    ignore: ['node_modules/**', 'coverage/**', 'build/**', '.next/**', 'scripts/**'],
  });
