...
async function testOnboardingFlow() {
  ...
  try {
    ...
    for (const interest of interests) {
      if (!interest.tag_id) {
        console.warn(
          `Skipping interest insert because tag_id is missing for one of the slugs: ${tagSlugs.join(', ')}`
        );
        continue;
      }
      const { error: interestError } = await supabase.from('user_interests').upsert({
        user_id: userId,
    ...
    console.log(
      '✅ Interests set:',
      interests.map((i) => ({ strength: i.strength, tag_id: i.tag_id }))
    );
