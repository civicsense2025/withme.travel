...
const fixImports = (filePath) => {
  try {
    ...
    if (content.includes('DB_TABLES') || content.includes('DB_FIELDS') || content.includes('DB_ENUMS')) {
      // Import for legacy constants
      const legacyImport = /import\s+\{\s*(DB_TABLES|DB_FIELDS|DB_ENUMS)(?:,\s*(DB_TABLES|DB_FIELDS|DB_ENUMS))?(?:,\s*(DB_TABLES|DB_FIELDS|DB_ENUMS))?\s*\}\s*from\s*['"]@\/utils\/constants\/database['"];?/g;
    ...
    if (content.includes('createSupabaseServerClient') || content.includes('createApiClient')) {
      content = content
        .replace(/import\s+\{\s*createSupabaseServerClient\s*\}\s*from\s*['"]@\/utils\/supabase\/server['"];?/g, 
          'import { createServerSupabaseClient } from "@/utils/supabase/server";')
        .replace(/import\s+\{\s*createApiClient\s*\}\s*from\s*['"]@\/utils\/supabase\/server['"];?/g, 
          'import { createServerSupabaseClient } from "@/utils/supabase/server";')
        .replace(/import\s+\{\s*createRouteHandlerClient\s*\}\s*from\s*['"]@\/utils\/supabase\/server['"];?/g, 
          'import { createServerSupabaseClient } from "@/utils/supabase/server";')
    ...
    if (content.includes('TRIP_ROLES') && !content.includes('import { TRIP_ROLES }')) {
      ...
      if (!content.includes('import { ENUMS }')) {
        if (content.includes('from "@/utils/constants/database"')) {
          content = content.replace(/import\s+\{([^}]*)\}\s*from\s*["']@\/utils\/constants\/database["'];?/g, 
    ...
    if (filePath.includes('/api/') && filePath.includes('/route.ts') && content.includes('params.')) {
      content = content.replace(/const\s+\{([^}]+)\}\s*=\s*params;/g, 
        'const {$1} = await params;');
      content = content.replace(/params\.([a-zA-Z0-9_]+)/g, 
        '(await params).$1');
      modified = true;
    }
    ...
    if (content.includes('searchParams.')) {
      content = content.replace(/searchParams\.get\(/g, 
        'searchParams?.get(');
      content = content.replace(/searchParams\.forEach\(/g, 
