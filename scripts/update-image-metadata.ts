...
import {
  getDestinationPhoto as getUnsplashPhoto,
  UnsplashPhoto,
...
type ImageMetadataUpsert = Partial<ImageMetadata> &
  Required<Pick<ImageMetadata, 'entity_id' | 'entity_type' | 'url' | 'source' | 'source_id'>>;
...
if (!PEXELS_API_KEY && !UNSPLASH_ACCESS_KEY) {
  console.error(
    chalk.red(
      'Error: Neither PEXELS_API_KEY nor UNSPLASH_ACCESS_KEY found in .env.local. At least one is required.'
    )
  );
  process.exit(1);
}
...
async function updateDestinationImageMetadata() {
  try {
    ...
    if (targetCities) {
      ...
      console.log(
        'No specific cities provided via --cities flag. Fetching all destinations from Supabase...'
      );
    ...
    for (const [index, destination] of destinations.entries()) {
      ...
      try {
        ...
        const hasExistingData =
          ...
          (destination.image_metadata?.source === 'unsplash' ||
            destination.image_metadata?.source === 'pexels') &&
          destination.image_metadata?.source_id;

        if (hasExistingData && !overwriteFlag) {
          console.log(
            chalk.gray('  Already has image metadata, skipping (use --overwrite to force update).')
          );
          continue;
        }
        ...
        console.log(
          chalk.dim(
            `  Attempting image search via ${useUnsplash ? 'Unsplash' : usePexels ? 'Pexels' : 'No Service Available'}...`
          )
        );
        ...
        if (result) {
          ...
          if (updateError) {
            console.error(
              chalk.red(`    Failed to update Supabase for ${locationString}:`),
              updateError.message
            );
          } else {
            console.log(
              chalk.green(`    Successfully updated metadata and image for ${locationString}.`)
            );
            processedCount++;
          }
        } else {
          console.warn(
            chalk.yellow(
              `  Could not find any suitable image for ${locationString} from any source.`
            )
          );
        }
      } catch (err) {
        ...
        console.error(
          chalk.red(`  Error processing ${locationString}: ${error.message}`),
          error.stack
        );
...
console.log(
  'Starting image metadata update script (with Unsplash & Pexels)...' +
