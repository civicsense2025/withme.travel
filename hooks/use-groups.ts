...
import { listGroups, createGroup, updateGroup, deleteGroup, Group, CreateGroupData, UpdateGroupData } from '@/lib/client/groups';
...
export function useGroups(guestToken?: string) {
  ...
  // Create group
  const handleCreateGroup = useCallback(async (data: CreateGroupData) => {
    try {
      setIsLoading(true);
  ...
  // Update group
  const handleUpdateGroup = useCallback(async (groupId: string, data: UpdateGroupData) => {
    try {
      setIsLoading(true);
