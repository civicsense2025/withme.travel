...
export function useInvitation(options: UseInvitationOptions = {}) {
  ...
  const fetchInvitation = useCallback(async (token: string, type?: 'trip' | 'group' | 'referral') => {
    setIsLoading(true);
    setError(null);
  ...
  const acceptTripInvitation = useCallback(async (token: string) => {
    setIsLoading(true);
    setError(null);
  ...
  const acceptGroupInvitation = useCallback(async (token: string) => {
    setIsLoading(true);
    setError(null);
