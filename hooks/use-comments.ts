...
export function useComments({
   ...
   */
  const fetchComments = useCallback(async (offset: number = 0, limit: number = initialLimit) => {
    setIsLoading(true);
    setError(null);
    ...
    try {
      ...
      if (contentType === 'group_idea') {
        ...
        response = await fetch(`/api/group-idea-comments?${queryParams.toString()}`);
      } else {
        const queryParams = new URLSearchParams({
          contentType,
          contentId,
      ...
      const errorMessage = err instanceof Error ? err.message : 'An error occurred while fetching comments';
   ...
   * Add a new comment
   */
  const addComment = useCallback(async (content: string, parentId?: string | null): Promise<CommentWithUser | null> => {
    setError(null);
    ...
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : 'An error occurred while adding comment';
      setError(errorMessage);
      toast({
        title: "Comment Error",
   ...
   */
  const getReplies = useCallback(async (commentId: string): Promise<CommentWithUser[]> => {
    setError(null);
    
    try {
      let response;
      if (contentType === 'group_idea') {
        response = await fetch(`/api/group-idea-comments/${commentId}/replies`);
      } else {
      ...
      const errorMessage = err instanceof Error ? err.message : 'An error occurred while fetching replies';
      setError(errorMessage);
   ...
   */
  const getReactions = useCallback(async (commentId: string): Promise<CommentReactionWithUser[]> => {
    setError(null);
    ...
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : 'An error occurred while fetching reactions';
      setError(errorMessage);
