...
type UseItineraryItemsReturn = {
  ...
  addItem: (newItemData: Omit<ItineraryItem, 'id' | 'trip_id' | 'created_at' | 'position' | 'votes' | 'user_vote'> & { day_number: number | null }) => Promise<ItineraryItem | null>; // Explicit return type
  updateItem: (itemId: string, updatedData: Partial<Omit<ItineraryItem, 'id' | 'trip_id' | 'created_at' | 'votes' | 'user_vote'>>) => Promise<void>;
...
export function useItineraryItems(tripId: string): UseItineraryItemsReturn {
  ...
  const addItem = useCallback(async (newItemData: Omit<ItineraryItem, 'id' | 'trip_id' | 'created_at' | 'position' | 'votes' | 'user_vote'> & { day_number: number | null }): Promise<ItineraryItem | null> => {
  ...
  const updateItem = useCallback(async (itemId: string, updatedData: Partial<Omit<ItineraryItem, 'id' | 'trip_id' | 'created_at' | 'votes' | 'user_vote'>>) => {
  ...
  const deleteItem = useCallback(async (itemId: string) => {
    ...
    setItemsByDay(prev => {
        const newItemsByDay: ItemsByDay = JSON.parse(JSON.stringify(prev));
        newItemsByDay[dayKey!] = newItemsByDay[dayKey!].filter((item: ItineraryItem) => item.id !== itemId);
         // Optional: Re-index positions if needed
         if (newItemsByDay[dayKey!]) {
            newItemsByDay[dayKey!] = newItemsByDay[dayKey!].map((item: ItineraryItem, index: number) => ({
                ...item,
                position: index
            }));
        }
