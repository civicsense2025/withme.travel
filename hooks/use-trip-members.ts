...
interface UseTripMembersReturn {
  ...
  importMembers: (members: AddTripMemberData[]) => Promise<{ added: number; invited: number } | null>;
  checkAccess: (userId: string, email?: string) => Promise<{ isMember: boolean; isInvited: boolean } | null>;
}
...
export function useTripMembers({
  ...
  const getMember = useCallback(async (userId: string): Promise<TripMember | null> => {
  ...
  // Add a new member
  const addMember = useCallback(async (data: AddTripMemberData): Promise<TripMember | null> => {
    if (!tripId) return null;
  ...
  const checkAccess = useCallback(
    async (userId: string, email?: string): Promise<{ isMember: boolean; isInvited: boolean } | null> => {
      if (!tripId) return null;
