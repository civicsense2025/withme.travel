    ...
    return (
      <main className="flex items-center justify-center min-h-screen bg-surface-light dark:bg-surface-light/10">
        <div className="max-w-xl mx-auto py-16 px-6 text-center">
          ...
          <p className="text-lg text-muted mb-8">{error || 'This referral link is invalid or has expired. If you think this is a mistake, ask your friend to send a new one!'}</p>
    ...
    <main className="min-h-screen flex flex-col md:flex-row">
      ...
      <div className="flex-1 flex flex-col justify-center items-center px-10 py-16 bg-primary-bg">
        <div className="w-full max-w-md flex flex-col items-center gap-8">
          {inviter.avatarUrl ? (
            <img src={inviter.avatarUrl} alt={inviter.name + ' avatar'} className="w-20 h-20 rounded-full border-2 border-accent-purple shadow-md mb-2" />
          ) : (
            <div className="w-20 h-20 rounded-full border-2 border-accent-purple bg-accent-purple/20 flex items-center justify-center shadow-md mb-2">
              <span className="text-accent-purple text-2xl font-bold">{inviter.name.substring(0, 2).toUpperCase()}</span>
            </div>
          )}
          <h1 className="text-3xl font-extrabold text-balance mb-2">{inviter.name} invited you to join <span className="text-accent-purple">withme.travel</span></h1>
          ...
          {!currentUser ? (
            <>
              <a href={`/signup?referral=${token}`} className="inline-block bg-accent-purple text-white font-semibold rounded-full px-8 py-3 text-lg shadow-lg hover:bg-accent-purple/90 transition-colors mb-2">Sign up and start planning</a>
              <div className="text-sm text-muted">Already have an account? <a href={`/login?redirect=/dashboard&referral=${token}`} className="text-accent-blue underline font-medium">Log in</a></div>
            </>
          ) : (
            <button 
