    ...
    return (
      <main className="min-h-screen flex items-center justify-center">
        <div className="max-w-xl mx-auto py-16 px-6 text-center">
          ...
          <p className="text-lg text-muted mb-8">{error || 'This invitation link is invalid or has expired. If you think this is a mistake, ask your friend to send a new one!'}</p>
          <a href="/" className="text-accent-purple font-medium underline">Back to home</a>
        </div>
      </main>
    );
  }
    ...
    <main className="min-h-screen flex flex-col md:flex-row">
      ...
      <div className="relative flex-1 flex flex-col justify-center items-center bg-surface-light dark:bg-surface-light/10 px-10 py-16 overflow-hidden">
        ...
        <div className="relative z-10 flex flex-col items-center gap-6">
          ...
          <h2 className="text-4xl font-extrabold text-primary-text text-center mb-2">withme.travel</h2>
          <p className="text-xl text-secondary-text text-center max-w-xs mb-4">Plan trips with friends—real collaboration, zero chaos.</p>
        </div>
      </div>
      {/* Right: Invite Details */}
      <div className="flex-1 flex flex-col justify-center items-center px-10 py-16 bg-primary-bg">
        <div className="w-full max-w-md flex flex-col items-center gap-8">
          <h1 className="text-3xl font-extrabold text-balance mb-2">Join <span className="text-accent-purple">{invitation.trip.name}</span> on withme.travel</h1>
          <p className="text-lg text-muted mb-4">
            <span className="font-semibold">{invitation.inviter.name}</span> invited you to this trip.
          </p>
            ...
            <div className="flex gap-4 mb-6">
              {invitation.trip.members.map((member: any, i: number) => (
                <div key={i} className="flex flex-col items-center">
                  {member.avatarUrl ? (
                    <img src={member.avatarUrl} alt={member.name + ' avatar'} className="w-12 h-12 rounded-full border-2 border-accent-purple" />
                  ) : (
                    <div className="w-12 h-12 rounded-full border-2 border-accent-purple bg-accent-purple/20 flex items-center justify-center">
                      <span className="text-accent-purple text-sm font-bold">{member.name.substring(0, 2).toUpperCase()}</span>
                    </div>
                  )}
          ...
          {!currentUser ? (
            <>
              <a href={`/signup?invitation=${token}`} className="inline-block bg-accent-purple text-white font-semibold rounded-full px-8 py-3 text-lg shadow-lg hover:bg-accent-purple/90 transition-colors mb-2">Sign up to join this trip</a>
              <div className="text-sm text-muted">Already have an account? <a href={`/login?redirect=/invite/${token}`} className="text-accent-blue underline font-medium">Log in</a></div>
            </>
          ) : (
            <button 
          ...
          <div className="mt-10 bg-surface-light dark:bg-surface-light/10 rounded-2xl p-6 shadow-md w-full">
            ...
            <p className="text-base text-secondary-text">Plan trips, organize groups, and make travel planning fun again. WithMe is your friendly, collaborative space for group adventures—no more endless group chats or lost docs. Jump in and start planning together!</p>
          </div>
        </div>
      </div>
    </main>
