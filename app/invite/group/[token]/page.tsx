  ...
  if (error || !invitation) {
    return (
      <main className="max-w-xl mx-auto py-16 px-6 text-center">
        ...
        <p className="text-lg text-muted mb-8">{error || 'This group invitation link is invalid or has expired. If you think this is a mistake, ask your friend to send a new one!'}</p>
      ...
      <div className="flex flex-col items-center gap-8">
        {invitation.group.avatarUrl ? (
          ...
          <div className="w-24 h-24 rounded-full shadow-md mb-2 bg-accent-purple/20 flex items-center justify-center">
            <span className="text-accent-purple text-2xl font-bold">{invitation.group.name.substring(0, 2).toUpperCase()}</span>
          </div>
        )}
        <h1 className="text-3xl font-extrabold text-balance mb-2">Join <span className="text-accent-purple">{invitation.group.name}</span> on withme.travel</h1>
        <p className="text-lg text-muted mb-4">
          <span className="font-semibold">{invitation.inviter.name}</span> invited you to join this group.
        </p>
        ...
        {!currentUser ? (
          <>
            <a href={`/signup?group_invitation=${token}`} className="inline-block bg-accent-purple text-white font-semibold rounded-full px-8 py-3 text-lg shadow-lg hover:bg-accent-purple/90 transition-colors mb-2">Sign up to join this group</a>
            <div className="text-sm text-muted">Already have an account? <a href={`/login?redirect=/invite/group/${token}`} className="text-accent-blue underline font-medium">Log in</a></div>
          </>
        ) : (
          <button 
