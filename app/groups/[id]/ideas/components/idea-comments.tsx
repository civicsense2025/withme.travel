...
export default function IdeaComments({ ideaId, userId, className = '', isAuthenticated, groupId }: IdeaCommentsProps) {
  ...
  const handleLoadReactions = useCallback(async (commentId: string) => {
  ...
  const Comment = useCallback(({ comment, isReply = false }: { comment: CommentWithUser, isReply?: boolean }) => {
            ...
            {!isReply && (comment.replies_count || 0) > 0 && (
              ...
              >
                {loadingReplies[comment.id] ? (
                  ...
                  <>
                    {showReplies ? 'Hide replies' : `Show ${comment.replies_count} ${comment.replies_count === 1 ? 'reply' : 'replies'}`}
                  </>
                )}
              </button>
            )}
    ...
    reactionsMap, editingId, replyingTo, userId, repliesMap, 
    handleLoadReactions, updateComment, deleteComment, handleAddReaction, handleSubmitReply, 
    handleToggleReplies, loadingReplies, getReplies
  ]);
        ...
        <div className="">
          <div className="flex items-center justify-between mb-2">
            <div className="font-semibold text-sm text-gray-700 dark:text-gray-200 flex items-center gap-1">
              ...
              <span className="font-bold text-base">Comments <span>({totalComments ?? 0})</span></span>
            </div>
            <Button
              size="icon"
              variant="ghost"
