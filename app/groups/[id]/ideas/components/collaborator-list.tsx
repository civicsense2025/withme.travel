    ...
    <Sheet open={open} onOpenChange={() => onClose()}>
      <SheetContent className="w-[350px] sm:w-[450px] overflow-y-auto p-5" side="right">
        <SheetHeader className="mb-5">
          ...
          <SheetDescription>
            See who's currently working on the whiteboard
          </SheetDescription>
        </SheetHeader>
        ...
        {sortedUsers.length === 0 ? (
          ...
          >
            ...
            <p className="max-w-xs">
              You're the only one working on this board right now. Invite others to collaborate in real-time!
            </p>
          </motion.div>
        ) : (
          <div className="space-y-4">
            <div className="flex justify-between text-sm text-muted-foreground mb-2 px-2">
              <span>{sortedUsers.length} collaborator{sortedUsers.length !== 1 ? 's' : ''}</span>
              <span>Status</span>
            </div>
        ...
        <div className="flex items-center gap-3">
          <div className={`relative rounded-full p-0.5 bg-gradient-to-r ${gradient}`}>
            <Avatar className="h-12 w-12 border-2 border-white">
              ...
              <AvatarFallback className="text-sm font-bold bg-white text-gray-800">{getInitials(user.name)}</AvatarFallback>
            </Avatar>
            {isOnline && (
          ...
          <div>
            <div className="font-medium flex items-center gap-1 text-base">
              {user.name || 'User'}
              {isCurrentUser && (
                <Badge variant="outline" className="ml-1 text-xs font-semibold">You</Badge>
              )}
            </div>
