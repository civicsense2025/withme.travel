...
export function ReadyForVotingModal({ onClose, groupId }: ReadyForVotingModalProps) {
  ...
  const getTotalMembers = () => {
    return Object.keys(activeUsers).length || 1;
  };
  ...
  const proceedToVoting = async () => {
    ...
    try {
      ...
      router.push(`/groups/${groupId}/ideas/vote`);
    } catch (error) {
      ...
      <DialogContent className="sm:max-w-md p-0 overflow-hidden">
        ...
        <div className="px-6 pt-6">
          <DialogHeader>
            ...
            <DialogDescription>
              When everyone is ready, you'll move to the voting phase to decide which ideas to include in your
      trip.
            </DialogDescription>
          </DialogHeader>
        </div>
        ...
        <div className="p-6">
          <div className="flex flex-col items-center justify-center mb-6">
            ...
            >
              <span className="text-lg font-bold text-purple-600">{getReadyCount()}</span> of <span className="text-lg font-bold text-gray-700">{getTotalMembers()}</span> people are ready
            </motion.p>
          </div>
          ...
          >
            ...
            {/* Other users */}
            {Object.entries(activeUsers).filter(([id]) => id !== user?.id).map(([id, userData], index) => {
                  ...
                  <div className="relative">
                    ...
                    {isUserReady && (
                      <motion.div 
                        ...
                        initial={{ scale: 0 }}
                        animate={{ scale: 1 }}
                        transition={{ type: "spring", stiffness: 500, damping: 15, delay: 0.6 + (index * 0.1) }}
                      >
                        <CheckCircle2 className="h-6 w-6 text-green-500" />
                      </motion.div>
                    )}
