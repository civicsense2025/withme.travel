...
export function CreateTripModal({
  ...
  const ideasByType = selectedIdeas.reduce((acc, idea) => {
    if (!acc[idea.type]) {
      acc[idea.type] = [];
    }
    ...
    try {
      ...
      const { data: tripData, error: tripError } = await supabase
        ...
        .insert({
          ...
          group_id: groupId,
          start_date: getDateFromIdeas(),
          end_date: null, // Could calculate from date ideas
          status: 'planning'
        })
        .select()
        .single();
      ...
      const otherIdeas = selectedIdeas.filter(idea => 
        !['destination', 'activity'].includes(idea.type)
      );
      ...
      if (otherIdeas.length > 0) {
        const tripNotes = otherIdeas.map(idea => ({
          trip_id: tripData.id,
          content: `# ${idea.title}\n${idea.description || ''}`,
          created_by: null // Would normally be the current user
        }));
    ...
    <Dialog open={true} onOpenChange={() => !isCreating && onClose()}>
      <DialogContent className="sm:max-w-[600px] max-h-[85vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Turn your group's ideas into an exciting new trip.
          </DialogDescription>
        </DialogHeader>
