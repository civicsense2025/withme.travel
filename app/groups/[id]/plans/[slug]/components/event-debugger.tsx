...
export const EventDebugger = () => {
  ...
  useEffect(() => {
    ...
    const getElementPath = (element: HTMLElement): string => {
      ...
      if (element.id) {
        selector += '#' + element.id;
      } else if (element.className && typeof element.className === 'string') { // Ensure className is a string
      ...
      while (parent && parent !== document.body && level < 3) {
        ...
        } else if (parent.className && typeof parent.className === 'string') { // Ensure className is a string
  ...
  return (
    ...
    >
      <div><strong>Last Click Target:</strong></div>
      <div style={{ wordBreak: 'break-all' }}>{clickTarget}</div>
      {lastEvent && (
        <div>
          <div><strong>Position:</strong> {lastEvent.x}, {lastEvent.y}</div>
          <div><strong>Element:</strong> {lastEvent.target}</div>
        </div>
      )}
    </div>
  );
