...
export function IdeasBoardHelpDialog({ open, onOpenChange }: IdeasBoardHelpDialogProps) {
  ...
  const shortcuts = [
    { key: 'D', action: 'Add Destination', color: 'bg-blue-50', textColor: 'text-blue-700', emoji: '📍' },
    { key: 'T', action: 'Add Date', color: 'bg-yellow-50', textColor: 'text-yellow-700', emoji: '📅' },
    { key: 'A', action: 'Add Activity', color: 'bg-green-50', textColor: 'text-green-700', emoji: '🏄‍♂️' },
    { key: 'B', action: 'Add Budget', color: 'bg-orange-50', textColor: 'text-orange-700', emoji: '💰' },
    { key: 'O', action: 'Add Other', color: 'bg-purple-50', textColor: 'text-purple-700', emoji: '💭' },
  ];
  ...
  const utilShortcuts = [
    ...
    { key: 'Ctrl+Shift+Enter', action: 'Ready for voting', color: 'bg-gray-50', textColor: 'text-gray-700' },
  ];
            ...
            <div className="grid grid-cols-1 gap-2">
              {utilShortcuts.map((shortcut, index) => (
                ...
                >
                  ...
                  <kbd className={`px-2 py-1 rounded-lg ${shortcut.color} ${shortcut.textColor} font-mono text-xs`}>
                    {shortcut.key}
                  </kbd>
                </motion.div>
              ))}
