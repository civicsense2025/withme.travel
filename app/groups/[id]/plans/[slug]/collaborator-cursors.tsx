...
function useUniqueActiveUsers<T extends { id: string; lastSeen?: Date }>(rawActiveUsers: T[] = []): T[] {
  return React.useMemo(() => {
    const validUsers = rawActiveUsers.filter(user => user && typeof user.id === 'string' && user.id.length > 0);
...
const CursorBubble = React.memo(function CursorBubble({ user, containerRect }: { user: ActiveUser; containerRect: DOMRect | null }) {
    ...
    >
      ...
      >
        {user.avatar_url ? (
          ...
          <div 
            className="w-5 h-5 rounded-full bg-white/30 flex items-center justify-center text-[10px] uppercase"
          >
            {user.name.charAt(0)}
          </div>
        )}
        ...
        {user.editing_idea_id && (
          <span className="ml-1 bg-white/20 px-1.5 py-0.5 rounded-sm text-[10px]">
            Editing
          </span>
        )}
      </motion.div>
...
export default function CollaboratorCursors({ containerRef, showCursors = true }: CollaboratorCursorsProps) {
  ...
  if (!uniqueUsers.length) {
    return <div className="absolute top-2 left-2 text-xs text-gray-400 pointer-events-none">No collaborators online</div>;
  }

  const maxCursors = 10;
  const visibleUsers = showCursors ? uniqueUsers.slice(0, maxCursors) : uniqueUsers.filter(u => u.id === currentUserId);
