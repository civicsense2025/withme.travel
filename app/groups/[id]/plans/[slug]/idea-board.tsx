...
import { GroupIdea as LocalGroupIdea, ColumnId as LocalColumnId, IdeaPosition as LocalIdeaPosition } from './store/idea-store';
...
export function IdeaBoard({ groupId, initialIdeas = [], isAuthenticated }: IdeaBoardProps) {
  ...
  const handleVote = useCallback(
    async (ideaId: string, voteType: 'UP' | 'DOWN') => {
      ...
      try {
        const response = await fetch(`/api/groups/${groupId}/plans/${groupId}/ideas/${ideaId}/vote`, {
          method: 'POST',
    ...
    >
      ...
      >
        {ideas.map(idea => {
          ...
          const isValidIdeaPosition = (pos: any): pos is LocalIdeaPosition => pos && typeof pos.columnId === 'string' && typeof pos.index === 'number';
          const position: LocalIdeaPosition = isValidIdeaPosition(idea.position) ? idea.position : { columnId: 'destination', index: 0 };
          return (
            <div key={idea.id}>
              <IdeaCard
                idea={{ ...(idea as unknown as LocalGroupIdea), created_by: idea.created_by ?? null, position: isValidIdeaPosition(idea.position) ? idea.position : { columnId: 'destination', index: 0 } }}
                position={position}
