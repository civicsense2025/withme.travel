...
export default function DestinationsPage() {
  ...
  const continents: string[] = Array.from(
    new Set(
      destinations.map((dest) => dest.continent).filter((c): c is string => typeof c === 'string') // Filter out null/undefined
    )
  ).sort();
      ...
      {isLoading ? (
        ...
        <div className="text-center py-16">
          ...
          <p className="text-muted-foreground mt-3">
            try adjusting your search or exploring different regions
          </p>
        </div>
      ) : (
        <>
