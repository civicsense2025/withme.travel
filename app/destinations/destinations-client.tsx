...
function groupByContinentAndCountry(destinations: Destination[]) {
  ...
  if (destinations.length > 0) {
    console.log(`[groupByContinentAndCountry] First destination: ${JSON.stringify(destinations[0])}`);
  }
  ...
  console.log(`[groupByContinentAndCountry] Found ${continents.length} continents: ${continents.join(', ')}`);
...
function DestinationsClient({ destinations }: DestinationsClientProps) {
  ...
  if (!continents.length) {
    return <div className="max-w-screen-2xl mx-auto px-4 md:px-8 py-8"><p>No destinations found.</p></div>;
  }
  ...
  console.log(`[DestinationsClient] Displaying ${allDestinationsToShow.length} out of ${destinations.length} destinations`);
          ...
          <>
            ...
            >
              {allDestinationsToShow.map((destination, index) => {
                ...
                if (index < 5) {
                  console.log(`[DestinationsClient] Destination ${index}: id=${destination.id}, name=${destination.name}, continent=${destination.continent}`);
                }
            ...
            {hasMoreToLoad && (
              <div className="mt-8 md:mt-12 flex justify-center">
                <Button onClick={handleLoadMore} variant="outline" className="rounded-full px-4 md:px-8 py-2 text-sm md:text-base">
