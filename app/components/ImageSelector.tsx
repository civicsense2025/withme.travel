  ...
  const searchImages = async () => {
    ...
    try {
      ...
      if (searchSource === 'pexels') {
        // Search Pexels API
        const response = await fetch(`/api/image-search/pexels?query=${encodeURIComponent(searchQuery)}`);
        const data = await response.json();
        results = data.photos || [];
      } else {
        // Search Unsplash API
        const response = await fetch(`/api/image-search/unsplash?query=${encodeURIComponent(searchQuery)}`);
        const data = await response.json();
