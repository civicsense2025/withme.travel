...
export function UserResponseSummary({
  ...
  const participationRate = memberCount > 0 
    ? Math.round((sessions.length / memberCount) * 100) 
    : 0;
  ...
  const responsesByQuestion = questions.reduce((acc, question) => {
    ...
    switch (question.type) {
      ...
      case QuestionType.YES_NO: {
        ...
        const chartData = Object.entries(counts).map(([label, count]) => ({
          name: question.type === QuestionType.YES_NO 
            ? (label === 'true' ? 'Yes' : 'No') 
            : (question as any).options?.find((o: any) => o.value === label || o.label === label)?.label || label,
          value: count
        }));
      ...
      case QuestionType.DRAG_RANK: {
        ...
        const averageRanks = Object.entries(ranks).map(([optionValue, rankList]) => {
          ...
          const optionLabel = (question as any).options?.find((o: any) => o.value === optionValue || o.id === optionValue)?.label || optionValue;
            ...
            <div className="h-60">
              <ResponsiveContainer width="100%" height="100%">
                ...
                >
                  ...
                  <Tooltip
                    formatter={(value) => [`${typeof value === 'number' ? value.toFixed(1) : value}`, 'Avg. Rank']}
      ...
      case QuestionType.BUDGET_ALLOCATOR: {
        ...
        const averageAllocations = Object.entries(allocations).map(([categoryId, amounts]) => {
          ...
          const categoryLabel = (question as any).categories?.find((c: any) => c.value === categoryId || c.id === categoryId)?.label || categoryId;
      ...
      case QuestionType.ACTIVITY_INTEREST: {
        ...
        const averageInterest = Object.entries(interestLevels).map(([activityId, levels]) => {
          ...
          const activityLabel = (question as any).activities?.find((a: any) => a.id === activityId)?.label || activityId;
            ...
            <div className="h-80">
              <ResponsiveContainer width="100%" height="100%">
                ...
                >
                  ...
                  <YAxis type="category" dataKey="name" width={100} />
                  <Tooltip 
                    formatter={(value) => [`${typeof value === 'number' ? value.toFixed(1) : value}`, 'Avg. Interest']}
                    labelFormatter={(value) => `${value}`}
                  />
                  <Bar dataKey="value" fill="#8884d8" />
                </BarChart>
        ...
        return (
          <div className="py-4">
            <div className="text-center text-sm">
              {questionResponses.length} {questionResponses.length === 1 ? 'response' : 'responses'} collected
            </div>
  ...
  const summarizableQuestions = questions.filter(q => 
    ![
    ...
    <div className="space-y-6">
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Summary of responses for {tripName}
          </CardDescription>
        </CardHeader>
