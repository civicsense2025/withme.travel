...
interface FeedbackFormRendererProps {
  form: FeedbackForm;
  questions: Question[];
  onSubmit: (responses: { formId: string; responses: { questionId: string; value: ResponseValue }[] }) => Promise<void>;
  onClose?: () => void;
}
...
export function FeedbackFormRenderer({
  ...
  const { handleSubmit, register, formState: { errors }, setValue, getValues, watch } = useForm({
            ...
            >
              {(question as any).options?.map((option: any) => (
                <div key={option.id} className="flex items-center space-x-2">
                  <RadioGroupItem value={option.value || option.label} id={`${fieldName}-${option.id}`} />
