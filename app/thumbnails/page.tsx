...
export default async function ThumbnailsDemoPage() {
  ...
  const tripIds = publicTrips.length > 0 
  ...
  const destinationData: DestinationData[] = popularDestinations.length > 0
    ...
    : [
        { city: 'Paris', country: 'France', byline: "The City of Lights awaits your adventure", slug: 'paris-france' },
        { city: 'Tokyo', country: 'Japan', byline: "Experience the blend of tradition and modernity", slug: 'tokyo-japan' },
        { city: 'New York', country: 'USA', byline: "The concrete jungle with endless possibilities", slug: 'new-york-usa' }
      ];
      ...
      <div className="pb-8">
        ...
        <div className="p-4 bg-blue-50 border border-blue-200 rounded-md mb-6">
          <p className="text-blue-700">
            <strong>Note:</strong> If thumbnails appear to be using the old design, you may need to hard refresh (Ctrl+F5 or Cmd+Shift+R) 
            or clear your browser cache to see the updated designs with padding.
          </p>
        </div>
        
        <div className="prose max-w-none">
          <p>
            Generate dynamic Open Graph images for social sharing. These images are automatically included in
            metadata for Twitter, Facebook, and other platforms that support Open Graph cards.
          </p>
        </div>
      </div>
          ...
          <Card>
            ...
            <CardFooter>
              <Button asChild className="w-full">
                <Link href="/api/thumbnail?title=Try%20It%20Yourself&subtitle=Create%20your%20own%20dynamic%20thumbnails" target="_blank">
                  Try It Yourself
                </Link>
              </Button>
            </CardFooter>
          ...
          <Card>
              ...
              {publicTrips.length > 0 ? (
                  ...
                  <div key={trip.id} className="space-y-3">
                    ...
                    <div className="flex justify-between items-center">
                      <div className="text-sm text-muted-foreground">
                        ...
                        {trip.start_date && <p>Dates: {new Date(trip.start_date).toLocaleDateString()}{trip.end_date && ` - ${new Date(trip.end_date).toLocaleDateString()}`}</p>}
                      </div>
                ...
                <div className="bg-muted p-4 rounded-md text-center">
                  ...
                  <div className="mt-4 flex justify-center space-x-4">
                    {[1, 2, 3].map(index => (
                      <div key={index} className="relative rounded-md overflow-hidden aspect-video shadow-md w-full max-w-xs">
                        <div className="absolute inset-0 bg-white">
                          <div className="absolute inset-[15%] bg-gradient-to-r from-blue-500 to-blue-700 rounded-md">
                            <div className="absolute top-4 left-4 text-white font-bold">withme.travel</div>
                            <div className="absolute bottom-12 left-4 text-white font-bold text-lg">Sample Trip {index}</div>
                            <div className="absolute bottom-6 left-4 text-white text-sm">Destination, Country</div>
            ...
            <CardFooter className="flex-col space-y-2">
              <p className="text-sm text-muted-foreground mb-2">
                Trip thumbnails feature our logo in the top left, with trip name, destination, dates, and tags displayed in an elegant layout at the bottom.
              </p>
              <Button asChild className="w-full">
                <Link href="/trips">
                  Explore Trips
        ...
        <TabsContent value="destinations" className="space-y-8">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Beautiful destination images with city/country information and highlighted attractions
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-8">
              {destinationData.map((destination, index) => (
                <div key={index} className="space-y-3">
                  <h3 className="text-lg font-medium">{destination.name || `${destination.city}, ${destination.country}`}</h3>
                  <div className="relative rounded-md overflow-hidden aspect-video shadow-md">
                    <Image 
      ...
      <div className="mt-16 max-w-3xl mx-auto">
        ...
        <div className="prose prose-sm mb-8">
          <p className="text-red-600 font-medium">
            Important: When sharing on social media, always use the direct API URLs shown in the code samples below,
            NOT the Next.js Image URLs that include _next/image in the path. The Next.js image URLs will not work for OG/social sharing.
          </p>
          <p>
            For example, use:
          </p>
        ...
        <div className="prose prose-sm">
          <p>
            Our dynamic thumbnail system generates Open Graph images on-the-fly using Next.js API routes. 
            These images are automatically included in the metadata for social sharing on platforms like 
          ...
          <ul>
            ...
            <li>Destination thumbnails featuring city information, country flags, and highlights</li>
          </ul>
        </div>
      </div>
    </div>
