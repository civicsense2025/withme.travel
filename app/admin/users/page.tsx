...
async function getUsers(supabase: SupabaseClient, page = 0, perPage = 20) {
...
export default async function AdminUsersPage({
  ...
  const { users, count, error } = await getUsers(supabase, page, perPage);
            ...
            <div className="hidden sm:flex sm:flex-1 sm:items-center sm:justify-between">
              <div>
                <p className="text-sm text-gray-700 dark:text-gray-300">
                  Showing <span className="font-medium">{page * perPage + 1}</span>{' '}
                  to <span className="font-medium">{Math.min((page + 1) * perPage, totalCount)}</span>{' '}
