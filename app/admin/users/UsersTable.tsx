...
export default function UsersTable({ initialData }: UsersTableProps) {
  ...
  const actions = [
    ...
    },
    {
      label: 'Delete',
      onClick: (rows: User[]) => {
        if (rows.length === 1 && confirm(`Are you sure you want to delete user ${rows[0].username || rows[0].email || rows[0].id}?`)) {
  ...
  const bulkActions = [
    ...
    },
    {
      ...
      confirmationMessage: 'Are you sure you want to delete the selected users? This cannot be undone.',
    },
  ];
