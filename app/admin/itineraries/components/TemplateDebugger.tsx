  ...
  return (
    <Card className="border-2 border-amber-200 dark:border-amber-800 rounded-2xl overflow-hidden bg-amber-50 dark:bg-amber-950/20">
      ...
      <CardContent className="p-4">
        ...
        <Accordion type="multiple" className="space-y-4" defaultValue={['template', 'tables', 'sections', 'items']}>
          <AccordionItem value="template" className="border rounded-lg overflow-hidden">
            ...
            <AccordionContent className="bg-white dark:bg-black border-t p-4">
              <div className="overflow-x-auto">
                {templateData ? (
                  <pre className="text-xs whitespace-pre-wrap">{JSON.stringify(templateData, null, 2)}</pre>
                ) : (
              ...
              {sectionsData.length > 0 ? (
                <div className="space-y-4">
                  {sectionsData.map((section, index) => (
                    <div key={section.id} className="border p-3 rounded bg-zinc-50 dark:bg-zinc-900">
                      <h4 className="font-medium mb-2">
                        Section {index + 1}: {section.title || `Day ${section.day_number}`}
                      </h4>
                      <pre className="text-xs overflow-x-auto">{JSON.stringify(section, null, 2)}</pre>
                    </div>
                  ))}
                </div>
              ) : (
          ...
          <AccordionItem value="execute" className="border border-amber-500 rounded-lg overflow-hidden">
