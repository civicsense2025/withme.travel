...
export default function TemplateSectionsEditor({
  ...
  const addNewSection = () => {
    const newDayNumber = sections.length > 0 
      ? Math.max(...sections.map(s => s.day_number)) + 1 
      : 1;
  ...
  const addNewItem = (sectionId: string | number, dayNumber: number) => {
    ...
    const newPosition = sectionItems.length > 0
      ? Math.max(...sectionItems.map(item => item.position || 0)) + 1
      : 0;
  ...
  const moveItemUp = (itemId: string) => {
    ...
    if (item.section_id !== prevItem.section_id || 
        item.day_number !== prevItem.day_number) {
      return;
    }
  ...
  const moveItemDown = (itemId: string) => {
    ...
    // Make sure they're in the same section/day
    if (item.section_id !== nextItem.section_id || 
        item.day_number !== nextItem.day_number) {
      return;
    }
  ...
  const getSectionItems = (sectionId: string | number, dayNumber: number) => {
    return items
      .filter(item => 
        item.section_id === sectionId || 
        (item.day_number === dayNumber && !item.section_id)
      )
  ...
  return (
    <div className="space-y-6">
      ...
      <Accordion type="multiple" className="w-full">
        {sections.sort((a, b) => a.day_number - b.day_number).map((section) => (
          <AccordionItem key={section.id} value={section.id.toString()}>
            <AccordionTrigger className="hover:bg-gray-50 dark:hover:bg-gray-900 px-4 rounded-md">
              <div className="flex-1 text-left">Day {section.day_number}: {section.title}</div>
            </AccordionTrigger>
              ...
              <div className="space-y-4 pt-4">
                ...
                <div>
                  <div className="flex justify-between items-center mb-4">
                    ...
                    <Button
                      size="sm"
                      onClick={() => addNewItem(section.id, section.day_number)}
                    >
                      <Plus className="h-4 w-4 mr-2" />
                      Add Activity
                    </Button>
                              ...
                              <TableCell>
                                <div className="space-y-1">
                                  ...
                                  <Input
                                    ...
                                    onChange={(e) => updateItem({ ...item, start_time: e.target.value })}
                                    className="text-sm"
                                  />
                                  ...
                                  <Input
                                    ...
                                    onChange={(e) => updateItem({ ...item, end_time: e.target.value })}
                                    className="text-sm"
                                  />
                                </div>
                              </TableCell>
        ...
        <Card>
          <CardContent className="p-8 text-center">
            <div className="text-gray-500 mb-4">
              No days have been added to this template yet
            </div>
            <Button onClick={addNewSection}>
              <Plus className="h-4 w-4 mr-2" />
              Add First Day
