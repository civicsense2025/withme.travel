      ...
      try {
                  ...
                  if (tripResponse.ok) {
                    ...
                    try {
                      ...
                      newStatuses.itineraryApi = {
                        ...
                        message: itineraryResponse.ok
                          ? 'Itinerary API working'
                          : 'Itinerary API error',
                      };
                    } catch (e) {
                      newStatuses.itineraryApi = {
        ...
        try {
          ...
          if (schemaResponse.ok) {
            ...
            if (missingTables.length === 0) {
              ...
              const missing = coreTables.filter((table) => missingTables.includes(table));
        ...
        try {
          ...
          if (cspHeader) {
            ...
            const hasSupabaseWs =
              cspHeader.includes('wss://*.supabase.co') ||
        ...
        newStatuses.serverSideAuth = {
          status:
            newStatuses.serverComponents.status === 'success' &&
        ...
        newStatuses.serverDataFetching = {
          status:
            newStatuses.tripApi.status === 'success' || newStatuses.tripApi.status === 'warning'
              ? 'success'
              : 'error',
          message:
            newStatuses.tripApi.status === 'success' || newStatuses.tripApi.status === 'warning'
              ? 'Server data fetching working'
              : 'Server data fetching issues',
        };
        ...
        newStatuses.clientDataFetching = {
          ...
          message:
            newStatuses.tripApi.status === 'success' || newStatuses.tripApi.status === 'warning'
              ? 'Client data fetching working'
              : 'Client data fetching issues',
        };

        // Database connection based on API success
        newStatuses.databaseConnection = {
          status:
            newStatuses.coreTables.status === 'success' ||
              ...
              {category.items.map((item) => (
                <li key={item} className="flex items-start">
                  ...
                  <div>
                    ...
                    <div
                      className={`text-sm ${getStatusColor(statuses[item as keyof StatusState].status)}`}
                    >
