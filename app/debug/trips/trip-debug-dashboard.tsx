...
export default function TripDebugDashboard() {
  ...
  const [tripLoadStatus, setTripLoadStatus] = useState<
  ...
  const addErrorLog = useCallback((message: string) => {
    setErrorLogs((prev) => [`[${new Date().toISOString()}] ${message}`, ...prev]);
  }, []);
  ...
  const fetchTripData = useCallback(async () => {
    ...
    setAuthChecks((prev) =>
      prev.map((check) => ({ ...check, status: 'pending', details: undefined }))
    );
    ...
    addBreadcrumb('Started trip data load debug', 'trip-debug', 'info', {
      tripId,
          ...
          prev.map((check) =>
            check.name === 'Authentication'
              ? {
                  ...check,
                  status: 'success',
                  details: `Authenticated as ${supabaseUser?.email ?? 'Unknown Email'}`,
                }
              : check
          )
        );
                ...
                prev.map((check) =>
                  check.name === 'Trip Access'
                    ? {
                        ...check,
                        status: 'error',
                        details: `Error checking access: ${tripExistsCheck.status}`,
                      }
                    : check
                )
              );
              addErrorLog(`Error checking trip access: HTTP ${tripExistsCheck.status}`);
              addBreadcrumb('Access check error', 'permissions', 'error', {
                status: tripExistsCheck.status,
              });
            }
          } else {
            ...
            prev.map((check) =>
              check.name === 'Trip Access'
                ? {
                    ...check,
                    status: 'error',
                    details: `Error: ${accessError instanceof Error ? accessError.message : 'Unknown error'}`,
                  }
                : check
            )
          );
          addErrorLog(
            `Error checking trip access: ${accessError instanceof Error ? accessError.message : 'Unknown error'}`
          );
  ...
  const fetchAndProcessEndpoint = useCallback(
    async (url: string, checkName: string, endpointIndex: number) => {
      ...
      try {
        ...
        addErrorLog(
          `Error fetching ${checkName}: ${error instanceof Error ? error.message : 'Unknown error'}`
        );
    ...
    const debugData = {
      ...
      data_loading: {
        ...
        itinerary_data: itineraryData
          ? {
              sections_count: Array.isArray(itineraryData.sections)
  ...
  const handleCopyDebugData = async () => {
    try {
      ...
      // Add error to logs
      addErrorLog(
        `Failed to copy debug data: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    } finally {
      setIsCopying(false);
    }
  ...
  const renderStatusChecks = (checks: StatusCheck[]) => {
    return checks.map((check) => (
      ...
      >
        ...
        {check.details && <div className="text-sm ml-7 text-gray-600">{check.details}</div>}
      </div>
    ));
  };
      ...
      <div className="grid grid-cols-1 gap-6">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Enter a Trip ID to diagnose data loading issues</CardDescription>
          </CardHeader>
          <CardContent>
              ...
              <div className="grid grid-cols-1 gap-3">
                <div className="flex flex-col space-y-1.5">
                  ...
                  <div className="flex w-full max-w-sm items-center space-x-2">
                    ...
                    />
                    <Button onClick={handleRunChecks} disabled={!isValidTripId || isLoading}>
                      {isLoading ? (
                        <>
            ...
            <Card>
              <CardHeader>
                <div className="flex items-center justify-between">
                  ...
                  {tripData && (
                    <Badge variant="outline">{tripData.name || tripData.id || 'Trip Loaded'}</Badge>
                  )}
                </div>
                <CardDescription>
                  ...
                  <div className="mt-4 border-t pt-4">
                    ...
                    <div className="space-y-2 text-sm">
                      ...
                      <div className="flex justify-between">
                        ...
                        <Badge variant="outline">{tripData.privacy_setting || 'private'}</Badge>
                      </div>
                      ...
                      <AccordionTrigger className="flex justify-between">
                        <div className="flex items-center gap-2">
                          ...
                          {endpoint.status === 'error' && (
                            <Badge variant="destructive">Error {endpoint.code}</Badge>
                          )}
          ...
          <TabsContent value="logs">
            <Card>
              <CardHeader>
                ...
                <CardDescription>Review error logs and diagnostic information</CardDescription>
              </CardHeader>
              <CardContent>
                  ...
                  <div>
                    ...
                    <div className="bg-gray-50 p-4 rounded-md max-h-60 overflow-y-auto">
                      {errorLogs.length > 0 ? (
                        ...
                        <div className="text-center text-gray-500 py-4">No error logs recorded</div>
                      )}
                    </div>
                  </div>
                      ...
                      {sentryBreadcrumbs.length > 0 ? (
                        <ul className="space-y-2">
                          {sentryBreadcrumbs.map((crumb, index) => (
                            <li
                              key={index}
                              className="border-b border-gray-200 pb-2 last:border-0 text-xs"
                            >
                              <div className="flex items-center gap-2">
                                {crumb.level === 'info' && (
