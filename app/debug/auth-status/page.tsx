          ...
          <Card>
            ...
            <CardContent>
              <div className="space-y-6">
                <div className={`p-4 rounded-md ${authState.guestToken ? 'bg-green-50 border border-green-200' : 'bg-gray-50 border border-gray-200'}`}>
                  <h3 className="text-lg font-medium mb-2">
                    Guest Token Status
                  </h3>
                  <p>
                    {authState.guestToken 
                      ? 'Guest token is present' 
                      : 'No guest token is present'}
                  </p>
                  {authState.guestToken && (
                    <div className="mt-4 space-y-2">
                      <div><strong>Token:</strong> {authState.guestToken.substring(0, 8)}...</div>
                      {authState.guestInfo && (
                        <>
                          <div><strong>Name:</strong> {authState.guestInfo.name}</div>
                          {authState.guestInfo.email && <div><strong>Email:</strong> {authState.guestInfo.email}</div>}
                        </>
                      )}
                    </div>
                  )}
