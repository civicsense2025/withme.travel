export default function TripsLoading() {
  return (
    <div className="max-w-6xl mx-auto px-4 py-6">
      <header className="mb-12">
        <div className="h-9 w-48 bg-muted rounded animate-pulse mb-4"></div>
        <div className="flex justify-between items-center">
          <div className="h-5 w-56 bg-muted rounded animate-pulse"></div>
          <div className="flex items-center gap-3">
            <div className="h-9 w-24 bg-muted rounded animate-pulse"></div>
            <div className="h-9 w-28 bg-muted rounded animate-pulse"></div>
          </div>
        </div>
      </header>

      <section className="mb-12">
        <div className="h-7 w-40 bg-muted rounded animate-pulse mb-6"></div>
        <div className="space-y-6">
          {[1, 2].map((i) => (
            <div key={i} className="rounded-xl overflow-hidden border border-border animate-pulse">
              <div className="flex flex-col md:flex-row md:h-56">
                <div className="h-48 md:h-auto md:w-2/5 bg-muted"></div>
                <div className="p-6 flex-grow relative md:w-3/5">
                  <div className="h-6 w-3/4 bg-muted rounded mb-3"></div>
                  <div className="h-4 w-2/3 bg-muted rounded mb-5"></div>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    <div className="h-5 w-32 bg-muted rounded"></div>
                    <div className="h-5 w-28 bg-muted rounded"></div>
                    <div className="h-5 w-48 bg-muted rounded md:col-span-2"></div>
                  </div>
                </div>
              </div>
            </div>
          ))}
        </div>
      </section>

      <section>
        <div className="h-7 w-32 bg-muted rounded animate-pulse mb-6"></div>
        <div className="space-y-6">
          {[1].map((i) => (
            <div key={i} className="rounded-xl overflow-hidden border border-border animate-pulse">
              <div className="flex flex-col md:flex-row md:h-56">
                <div className="h-48 md:h-auto md:w-2/5 bg-muted"></div>
                <div className="p-6 flex-grow relative md:w-3/5">
                  <div className="h-6 w-3/4 bg-muted rounded mb-3"></div>
                  <div className="h-4 w-2/3 bg-muted rounded mb-5"></div>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    <div className="h-5 w-32 bg-muted rounded"></div>
                    <div className="h-5 w-28 bg-muted rounded"></div>
                    <div className="h-5 w-48 bg-muted rounded md:col-span-2"></div>
                  </div>
                </div>
              </div>
            </div>
          ))}
        </div>
      </section>
    </div>
  );
} 