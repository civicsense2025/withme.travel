...
function extractImageAttribution(url: string | undefined): { creator?: string; creatorUrl?: string; source?: 'unsplash' | 'pexels' | undefined } {
  if (!url) return {};
  ...
  try {
    ...
    if (url.includes('unsplash.com')) {
      ...
      const unsplashMatch = url.match(/unsplash\.com\/photos\/[^/]+\/@([^/?&]+)/i) || 
    ...
    if (url.includes('pexels.com')) {
      ...
      const pexelsMatch = url.match(/pexels\.com\/photo\/[^/]+\-([0-9]+)/i) ||
                         url.match(/pexels\.com\/@([^/?&]+)/i);
