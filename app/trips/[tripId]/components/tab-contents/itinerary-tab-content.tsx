...
const MapboxGeocoderComponent = dynamic(() => import('@/components/maps/mapbox-geocoder'), { 
  ssr: false,
  loading: () => <div className="p-2 border rounded text-sm text-muted-foreground">Loading location search...</div>
});
...
export function ItineraryTabContent({
  ...
  useEffect(() => {
    if (allItineraryItems.length !== itineraryItems.length || 
  ...
  const handleAddItemSubmit = useCallback(async (e: React.FormEvent) => {
    ...
    try {
      ...
      const dayDescription = selectedDayNumber !== null 
  ...
  }, [title, itemCategory, notes, selectedPlace, selectedDayNumber, tripId, toast, setAllItineraryItems, resetAddItemForm, allItineraryItems]);

  // Handle save edited item
  const handleSaveEditedItem = useCallback(async (updatedItem: DisplayItineraryItem) => {
    try {
      setIsLoading(true);
    ...
    <div className="w-full space-y-6">
      ...
      <Dialog open={isAddItemDialogOpen} onOpenChange={setIsAddItemDialogOpen}>
        <DialogContent className="sm:max-w-[500px]">
          <DialogHeader>
            <DialogTitle>
              {selectedDayNumber !== null ? `Add Item to Day ${selectedDayNumber}` : 'Add Unscheduled Item'}
            </DialogTitle>
            <DialogDescription>
