    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg p-4 w-11/12 max-w-4xl max-h-[90vh] flex flex-col">
          ...
          <div className="flex flex-col h-[400px] overflow-hidden">
            ...
            <div className="overflow-y-auto flex-1 border rounded p-2">
              {selectedPlaces.map((place) => (
                <div
                  key={place.id}
                  className="p-2 flex justify-between items-start border-b last:border-0"
                >
                  <div>
                    <div className="font-medium">{place.name}</div>
              ...
              {selectedPlaces.length === 0 && (
                <div className="text-gray-500 text-sm p-2">No places selected yet</div>
              )}
            </div>
          </div>
        </div>
        ...
        <div className="flex justify-end gap-2 mt-4">
          ...
          <Button onClick={handleImport} disabled={selectedPlaces.length === 0}>
