...
export function useTripPresence(tripId: string) {
  ...
  const fetchActiveUsers = useCallback(async () => {
    ...
    try {
      ...
      // Transform data to format expected by components
      const transformedData = data.map((presence: any) => ({
  ...
  const updateUserPresence = useCallback(async (status: string, itemId?: string) => {
    if (!tripId || !user) return;
