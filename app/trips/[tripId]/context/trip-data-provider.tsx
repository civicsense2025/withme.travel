...
export function TripDataProvider({ children, initialData, tripId }: TripDataProviderProps) {
  ...
  useEffect(() => {
    ...
    if (allFetchesDone) {
      ...
      console.log('[TripDataProvider] tripResponse', tripResponse);
      console.log('[TripDataProvider] itineraryResponse', itineraryResponse);
      console.log('[TripDataProvider] membersResponse', membersResponse);

      // Support both { trip: { ... }, ... } and { ...tripFields }
      const actualTripData = tripResponse?.trip ?? tripResponse;
      const actualItineraryData = itineraryResponse?.data ?? { sections: [], items: [] };
      const actualMembersData = membersResponse?.data ?? [];
