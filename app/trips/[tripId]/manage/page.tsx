...
import { ArrowLeft, Settings, Users, Share2, Trash2, Globe2, MapPin, CalendarIcon, Loader2 } from "lucide-react"
...
import { API_ROUTES, PAGE_ROUTES, TRIP_ROLES, DB_FIELDS } from "@/utils/constants"
...
export default function ManageTripPage({ params: { tripId } }: PageProps) {
  ...
  useEffect(() => {
    ...
    async function fetchTripData() {
      try {
        ...
        setIsAdmin(currentMember?.role === TRIP_ROLES.ADMIN || currentMember?.role === TRIP_ROLES.EDITOR)
  ...
  const DragHandle = ({ attributes, listeners, ref }: DragHandleProps) => (
                ...
                <div className="md:col-span-2">
                  <ScrollArea className="h-[300px] pr-4 border rounded-md">
                    <div className="p-4 space-y-4">
                      {members.length > 0 ? (
                        members.map((member) => (
                          <div key={member.id} className="flex items-center justify-between p-3 border rounded-md bg-background hover:bg-muted/50">
