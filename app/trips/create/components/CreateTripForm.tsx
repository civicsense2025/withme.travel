...
import { TABLES } from '@/utils/constants/database'; // Use correct aliases
...
export function CreateTripForm() {
  ...
  const onSubmit = async (values: FormValues) => {
    setLoading(true);
    try {
      console.log('Submitting trip creation form with values:', values);
      ...
      console.log('Trip creation response status:', response.status, response.statusText);
      ...
      if (!response.ok) {
        ...
        console.error('Trip creation failed with response:', errorData);
      ...
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
        <Card>
          <CardHeader>
            ...
            <CardDescription>Get started with a new trip or use a template below.</CardDescription>
          </CardHeader>
