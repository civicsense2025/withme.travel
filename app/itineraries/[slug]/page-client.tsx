...
import { 
  Heart, Share2, Calendar, MapPin, Clock, User, Star, Tag, Info, ArrowLeft,
  ChevronDown, PlusCircle, ListChecks 
} from 'lucide-react';
...
export default function ItineraryTemplatePageClient({
  ...
  const imageAttribution = template.destination?.image_metadata ? 
    createAttributionText(template.destination.image_metadata) : null;
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-16">
          ...
          >
            ...
            {template.metadata && Object.keys(template.metadata).length > 0 && (
              <>
                <h2 className="text-2xl font-medium mb-4">What to know about {template.destination?.city}, {template.destination?.country}</h2>
                <DestinationDetails 
            ...
            <div className="rounded-xl border border-gray-200 dark:border-gray-800 bg-card p-8 shadow-sm space-y-6">
              ...
              <div className="flex items-center space-x-4 mb-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">Creating exceptional itineraries since 2020</p>
                </div>
              </div>
              
              <p className="text-sm text-muted-foreground">
                This local expert creates personalized travel experiences based on extensive research and personal visits to each destination.
              </p>
