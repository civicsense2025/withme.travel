...
export async function PUT(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    if (updateError) {
      ...
      return NextResponse.json(
        { error: 'Failed to update destination', details: updateError.message },
        { status: 500 }
      );
    }

    return NextResponse.json(
      { success: true, message: 'Destination updated successfully' },
      { status: 200 }
    );
  } catch (error) {
    console.error('Error in destination update endpoint:', error);
    return NextResponse.json(
