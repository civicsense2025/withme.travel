...
export async function POST(request: NextRequest) {
  try {
    ...
    if (existingRequest) {
      return formatErrorResponse(new ApiError('A friend request already exists between these users', 400));
    }
    ...
    const { data: existingFriendship, error: friendshipError } = await supabase
      .from(TABLES.FRIENDS)
      .select('*')
      .or(`(user_id_1.eq.${user.id}.and.user_id_2.eq.${receiver_id}).or.(user_id_1.eq.${receiver_id}.and.user_id_2.eq.${user.id})`)
      .maybeSingle();
