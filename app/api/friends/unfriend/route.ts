...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data: friendship, error: friendshipError } = await supabase
      .from(TABLES.FRIENDS)
      .select('id')
      .or(`(user_id_1.eq.${user.id}.and.user_id_2.eq.${friend_id}).or.(user_id_1.eq.${friend_id}.and.user_id_2.eq.${user.id})`)
      .maybeSingle();
    ...
    await supabase
      ...
      .delete()
      .or(`(sender_id.eq.${user.id}.and.receiver_id.eq.${friend_id}).or.(sender_id.eq.${friend_id}.and.receiver_id.eq.${user.id})`);
