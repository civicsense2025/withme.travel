...
export async function POST(request: NextRequest) {
  try {
    ...
    const { error: associateError } = await supabase.from('entity_tags').insert({
    ...
    if (associateError) {
      ...
      return NextResponse.json({ error: 'Failed to associate tag with entity' }, { status: 500 });
    }

    return NextResponse.json({ success: true }, { status: 200 });
  } catch (error) {
    console.error('Error in tag-entity POST handler:', error);
    return NextResponse.json({ error: 'Failed to add tag to entity' }, { status: 500 });
  }
}
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    const { error: removeError } = await supabase.from('entity_tags').delete().match({
      entity_type: entityType,
      entity_id: entityId,
      tag_id: tag.id,
    });
    ...
    if (removeError) {
      ...
      return NextResponse.json({ error: 'Failed to remove tag from entity' }, { status: 500 });
    }
