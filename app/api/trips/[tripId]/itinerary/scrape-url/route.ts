...
async function checkUserPermission(supabase: any, tripId: string, userId: string): Promise<boolean> {
...
export async function POST(request: NextRequest, props: { params: { tripId: string } }) {
    ...
    try {
        ...
        const canEdit = role === TRIP_ROLES.ADMIN.toUpperCase() || role === TRIP_ROLES.EDITOR.toUpperCase();

        if (!canEdit) {
            return NextResponse.json({ error: 'You do not have permission to add itinerary items to this trip.' }, { status: 403 });
        }
        ...
        try {
            ...
            return NextResponse.json({ error: `Invalid request: ${parseError.message}` }, { status: 400 });
        }
        ...
        try {
            ...
            if (!contentType || !contentType.includes('text/html')) {
                 console.warn(`Scrape URL Warning: Content type is not HTML (${contentType}) for URL: ${urlToScrape}`);
            ...
            console.log(`Successfully fetched content from ${urlToScrape} (length: ${htmlContent.length})`);
        } catch (fetchError: any) {
            console.error(`Scrape URL Error: Failed to fetch ${urlToScrape}`, fetchError);
            return NextResponse.json({ error: `Failed to fetch the provided URL: ${fetchError.message}` }, { status: 500 });
        }

        // 4. Extract Metadata (using basic string matching)
        const scrapedData = {
            title: extractMetaContent(htmlContent, 'og:title') || extractTitle(htmlContent),
            description: extractMetaContent(htmlContent, 'og:description') || extractMetaContent(htmlContent, 'description'),
            imageUrl: extractMetaContent(htmlContent, 'og:image'),
            scrapedUrl: extractCanonicalUrl(htmlContent) || extractMetaContent(htmlContent, 'og:url') || urlToScrape,
        };
