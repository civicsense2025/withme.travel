...
export async function POST(
  ...
  try {
      ...
      for (const invitation of importData.invitations) {
        try {
          ...
          if (inviteError) {
            ...
            results.invitations.failed++
            results.invitations.errors.push(`Failed to invite ${invitation.email}: ${inviteError.message}`)
          } else {
      ...
      for (const member of importData.members) {
        try {
          ...
          const { count, error: checkError } = await supabase
            ...
            .eq(DB_FIELDS.TRIP_MEMBERS.TRIP_ID, tripId)
            .eq(DB_FIELDS.TRIP_MEMBERS.USER_ID, member.userId)
    ...
    console.error("Error importing Splitwise members:", error)
    return NextResponse.json(
