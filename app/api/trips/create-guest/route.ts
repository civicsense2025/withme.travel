...
export async function POST(request: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    if (user) {
      ...
      console.log(chalk.yellow(`${LOG_PREFIX} Using guest token: ${guestToken} with UUID: ${userId}`));
      isGuest = true;
    }
    ...
    if (isGuest) {
      ...
      if (!existingProfile) {
        ...
        if (insertProfileError) {
          console.error(chalk.red(`${LOG_PREFIX} Error inserting guest profile:`), insertProfileError);
    ...
    const tripTitle = customName || `New trip to ${cityName}${countryName ? `, ${countryName}` : ''}`;
    ...
    console.log(chalk.blue(`${LOG_PREFIX} Creating trip with data:`), JSON.stringify({
      ...tripData,
    ...
    if (tripError || !newTrip) {
      ...
      if (tripError?.code === '22P02' && tripError?.message?.includes('invalid input syntax for type uuid')) {
        console.log(chalk.yellow(`${LOG_PREFIX} Detected UUID syntax error, attempting modified insertion...`));
        ...
        console.log(chalk.dim(`${LOG_PREFIX} Retrying with simplified data:`, JSON.stringify({
          ...simplifiedTripData,
    ...
    if (isGuest && guestToken) {
      ...
      try {
        ...
        if (updateError) {
          console.log(chalk.yellow(`${LOG_PREFIX} Could not update trip with guest token: ${updateError.message}`));
          ...
          if (tokenError) {
            console.log(chalk.yellow(`${LOG_PREFIX} Could not store in guest_tokens: ${tokenError.message}`));
            // This is non-fatal, we'll still allow the trip to be created
          } else {
            console.log(chalk.green(`${LOG_PREFIX} Guest token association stored in guest_tokens table`));
          }
        } else {
    ...
    console.log(chalk.dim(`${LOG_PREFIX} Member data:`, JSON.stringify({
      ...memberData,
    ...
    if (memberError) {
      ...
      if (memberError.code === '22P02') {
        console.log(chalk.yellow(`${LOG_PREFIX} Attempting fallback insertion without guest_token...`));
    ...
    if (itemsError) {
      console.error(
        chalk.yellow(`${LOG_PREFIX} Error creating default items:`),
        itemsError
      );
      // Continue anyway - this is not fatal
    }
