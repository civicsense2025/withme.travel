...
export async function GET(request: Request) {
  try {
    ...
    return NextResponse.json(
      { error: error.message || "Failed to fetch Splitwise groups" },
      { status: 500 }
    );
  }
}
...
export async function POST(request: Request) {
  try {
    ...
    if (!tripId || !groupId) {
      return NextResponse.json(
        { error: "Trip ID and group ID are required" },
        { status: 400 }
      );
    }
    ...
    if (![TRIP_ROLES.ADMIN, TRIP_ROLES.EDITOR].includes(tripMembership.role as any)) {
      return NextResponse.json(
        { error: "You don't have permission to link Splitwise to this trip" },
        { status: 403 }
      );
    }
    ...
    try {
      ...
      if (error instanceof SplitwiseError && error.statusCode === 404) {
          return NextResponse.json(
            { error: "Splitwise group not found or not accessible" },
            { status: 404 }
          );
      ...
      return NextResponse.json(
        { error: "Failed to verify Splitwise group" },
        { status: 503 } // Indicate upstream issue
      );
    }
    ...
    if (!success) {
      ...
      return NextResponse.json(
        { error: "Failed to link trip to Splitwise group in database" },
        { status: 500 }
      );
    }
    ...
    return NextResponse.json(
      { error: error.message || "Failed to link trip to Splitwise group" },
      { status: 500 }
    );
  }
} 