...
import { storeSplitwiseCredentials, getCurrentUser, SplitwiseError } from "@/lib/services/splitwise";
...
export async function GET(request: Request) {
  ...
  const tripId = cookieStore.get("splitwise_trip_id")?.value;
  ...
  if (error) {
    ...
    return NextResponse.redirect(
      `${process.env.NEXT_PUBLIC_APP_URL}${redirectUrl}?error=splitwise_auth_failed&error_description=${encodeURIComponent(error)}`
    );
  }
  ...
  if (!code) {
    const redirectUrl = tripId ? PAGE_ROUTES.TRIP_DETAILS(tripId) : PAGE_ROUTES.TRIPS;
    return NextResponse.redirect(
      `${process.env.NEXT_PUBLIC_APP_URL}${redirectUrl}?error=missing_auth_code`
    );
  }
  ...
  try {
    ...
    if (authError || !user) {
      ...
      return NextResponse.redirect(
        `${process.env.NEXT_PUBLIC_APP_URL}/login?redirect=${encodeURIComponent(loginRedirect)}`
      );
    }
    ...
    if (!tokenResponse.ok) {
      ...
      const redirectUrl = tripId ? PAGE_ROUTES.TRIP_DETAILS(tripId) : PAGE_ROUTES.TRIPS;
      return NextResponse.redirect(
        `${process.env.NEXT_PUBLIC_APP_URL}${redirectUrl}?error=token_exchange_failed&splitwise_error=${encodeURIComponent(tokenData?.error || 'Unknown')}`
      );
    }
    ...
    if (!tokenData || typeof tokenData !== 'object' || !tokenData.access_token) {
      ...
      const redirectUrl = tripId ? PAGE_ROUTES.TRIP_DETAILS(tripId) : PAGE_ROUTES.TRIPS;
      return NextResponse.redirect(
        `${process.env.NEXT_PUBLIC_APP_URL}${redirectUrl}?error=invalid_token_data`
      );
    }
    ...
    try {
        ...
        if (!currentUserResponse.ok || !currentUserData?.user?.id) {
        ...
        splitwiseUserId = currentUserData.user.id;
         ...
         const redirectUrl = tripId ? PAGE_ROUTES.TRIP_DETAILS(tripId) : PAGE_ROUTES.TRIPS;
         return NextResponse.redirect(
            `${process.env.NEXT_PUBLIC_APP_URL}${redirectUrl}?error=fetch_splitwise_user_failed`
         );
    }
    ...
    const success = await storeSplitwiseCredentials(
      user.id,
      tokenData.access_token,
      refreshToken,
      expiresAt,
      splitwiseUserId // Use the fetched ID
    );
    ...
    if (!success) {
      ...
      const redirectUrl = tripId ? PAGE_ROUTES.TRIP_DETAILS(tripId) : PAGE_ROUTES.TRIPS;
      return NextResponse.redirect(
        `${process.env.NEXT_PUBLIC_APP_URL}${redirectUrl}?error=storing_credentials_failed`
      );
    }
    ...
    const redirectUrl = tripId ? PAGE_ROUTES.TRIP_DETAILS(tripId) : PAGE_ROUTES.TRIPS;
    return NextResponse.redirect(
      `${process.env.NEXT_PUBLIC_APP_URL}${redirectUrl}?splitwiseConnected=true`
    );
    ...
    return NextResponse.redirect(
      `${process.env.NEXT_PUBLIC_APP_URL}${redirectUrl}?error=${encodeURIComponent(errorMessage)}`
    );
  }
} 