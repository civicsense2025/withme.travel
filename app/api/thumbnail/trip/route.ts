...
export async function GET(
  ...
  try {
    ...
    const imageUrl = tripData.cover_image_url || 
                    (tripData.destinations?.image_url) || 
                    null;
    ...
    const destinationText = tripData.destinations && tripData.destinations.city && tripData.destinations.country
      ? `${tripData.destinations.city}, ${tripData.destinations.country}`
      : 'No destination specified';

    // Format tags if available with null safety
    const tagsText = tripData.tags && tripData.tags.length > 0
      ? tripData.tags.slice(0, 5).map(tag => `#${tag.toLowerCase()}`).join('  ')
      : '';
    ...
    if (admin && admin.profiles) {
      ...
      if (profileData && profileData.first_name) {
        ...
        if (profileData.last_name) {
          adminInitials = `${profileData.first_name[0] || ''}${profileData.last_name[0] || ''}`.toUpperCase();
        } else {
