...
// PATCH: /api/group-idea-comments (edit comment)
...
// DELETE: /api/group-idea-comments?id=...&user_id=...
export async function DELETE(request: NextRequest) {
  ...
  const { error } = await supabase.from('group_idea_comments').delete().eq('id', id);
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });
