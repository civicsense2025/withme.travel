...
export async function GET(): Promise<NextResponse> {
  try {
    ...
    // Handle null values appropriately for the function call
    const city = randomDestination.city || '';
    const country = randomDestination.country || '';
    const stateProvince = randomDestination.state_province || undefined;

    const result = await getRandomDestinationImage(city, country, stateProvince);
