...
function validateCategory(category?: string): PlaceCategory | undefined {
  ...
  const validCategories: PlaceCategory[] = [
    'attraction', 'restaurant', 'cafe', 'hotel', 
    'landmark', 'shopping', 'transport', 'other'
  ];
  ...
  if (['restaurant', 'dining', 'food', 'eatery'].some(term => normalizedCategory.includes(term))) {
    return 'restaurant';
  }
  
  if (['hotel', 'accommodation', 'lodging', 'stay'].some(term => normalizedCategory.includes(term))) {
    return 'hotel';
  }
  ...
  if (['attraction', 'sight', 'point of interest'].some(term => normalizedCategory.includes(term))) {
    return 'attraction';
  }
  ...
  if (['transportation', 'transit', 'station', 'airport'].some(term => normalizedCategory.includes(term))) {
    return 'transport';
  }
