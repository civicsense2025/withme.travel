...
module.exports = function transformer(file, api) {
    ...
    root.find(j.ImportDeclaration)
      ...
      .forEach(path => {
        path.node.specifiers = path.node.specifiers.filter(spec => {
          ...
          return !['TABLES', 'FIELDS', 'ENUMS', 'CommentableContentType', 'ItineraryTemplateMetadata'].includes(name);
        });
    ...
    root.find(j.VariableDeclarator)
      .filter(path => path.node.init && path.node.init.type === 'Identifier' && path.node.init.name === 'TABLES')
      .forEach(path => {
    ...
    root.find(j.VariableDeclarator)
      .filter(path => path.node.init && path.node.init.type === 'Identifier' && path.node.init.name === 'FIELDS')
      .forEach(path => {
        j(path.parent).remove();
      });
