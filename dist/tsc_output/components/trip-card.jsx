...
import { formatDateRange, getColorClassFromId } from "@/lib/utils";
...
export function TripCard({ trip }) {
    ...
    return (<Link href={PAGE_ROUTES.TRIP_DETAILS(trip.id)}>
      <motion.div className="rounded-3xl overflow-hidden h-full shadow-md hover:shadow-lg transition-all duration-300 max-w-xs mx-auto" whileHover={{ y: -5 }} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }}>
        <div className="relative h-48 w-full">
          <OptimizedImage metadata={imageMetadata} type="trip_cover" fallbackText={displayTitle} fill className="object-cover" showAttribution imageOptions={{
            width: 400,
            height: 200,
            quality: 80,
            format: 'webp'
        ...
        <div className={`p-4 ${colorClass} bg-opacity-30`}>
          ...
          <div className="space-y-2">
            {trip.start_date && trip.end_date && (<div className="flex items-center gap-1 text-sm">
