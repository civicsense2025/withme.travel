...
function MemberAvatars({ members }) {
    ...
    return (<div className="flex items-center -space-x-2" title={`${members.length} member${members.length !== 1 ? 's' : ''}`}>
      {visibleMembers.map((member, index) => {
            ...
            return (<Avatar key={profile.id} className="inline-block h-6 w-6 rounded-full ring-2 ring-background" title={name}>
            <AvatarImage src={profile.avatar_url || undefined} alt={name}/>
            <AvatarFallback>{initials}</AvatarFallback>
          </Avatar>);
        })}
      {hiddenCount > 0 && (<div className="flex items-center justify-center h-6 w-6 rounded-full bg-muted text-muted-foreground text-xs font-medium ring-2 ring-background z-10">
          +{hiddenCount}
        </div>)}
    </div>);
}
// Main TripHeader component
export function TripHeader({ title, description, tags, destination, startDate, endDate, members }) {
    return (<div className="space-y-2">
      {/* Trip Title */}
      <h1 className="text-3xl font-bold tracking-tight break-words">{title}</h1>
      
      {/* Trip Description (Optional) */}
      {description && (<p className="text-muted-foreground text-base mt-1">
          {description}
        </p>)}
      
      {/* Trip Tags (Optional) */}
      {tags && tags.length > 0 && (<div className="flex flex-wrap gap-2 pt-2">
          {tags.map((tag) => (<Badge key={tag} variant="secondary" className="font-normal">
