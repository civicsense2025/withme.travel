...
export function LikeButton({ itemId, itemType, className = '', variant = 'icon', size = 'md', showCount = false, count, onClick, }) {
    ...
    return (<>
      <Button variant={variant === 'icon' ? 'ghost' : variant} size={variant === 'icon' ? 'icon' : 'default'} className={cn(variant === 'icon' && getSizeClass(), 'group relative', 'bg-white dark:bg-gray-950', 'hover:bg-gray-100 dark:hover:bg-gray-900', 'border border-gray-200 dark:border-gray-800', 'shadow-sm', className)} onClick={handleToggleLike} disabled={isLoading}>
        {variant === 'icon' ? (<Heart className={cn(getHeartSize(), 'transition-all duration-300', isLikedState
                ? 'fill-red-500 text-red-500'
                : 'text-gray-500 dark:text-gray-400 group-hover:scale-110 group-hover:text-red-500')}/>) : (<div className="flex items-center gap-2">
            <Heart className={cn('h-4 w-4 transition-colors', isLikedState
                ? 'fill-red-500 text-red-500'
                : 'text-gray-500 dark:text-gray-400 group-hover:text-red-500')}/>
            <span className="text-gray-700 dark:text-gray-200">{isLikedState ? 'Liked' : 'Like'}</span>
