...
export function ItineraryDisplay({ initialItems, tripId, canEdit }) {
        ...
        setItems(currentItems => currentItems.map(item => {
            if (item.id === itemId) {
                ...
                if (currentUserVote === voteType) { // User is toggling off their vote
                    ...
                    return Object.assign(Object.assign({}, item), { votes: Object.assign(Object.assign({}, item.votes), { up: newUpVotes, down: newDownVotes, userVote: null }) });
                }
                    ...
                    if (currentUserVote === 'down')
                        newDownVotes--;
                    voteType === 'up' ? newUpVotes++ : newDownVotes++;
                    return Object.assign(Object.assign({}, item), { votes: Object.assign(Object.assign({}, item.votes), { up: newUpVotes, down: newDownVotes, userVote: voteType }) });
                }
            }
            return item;
        }));
    ...
    const renderVoters = (voters) => {
        ...
        {voters.slice(0, 5).map((voter) => (<Avatar key={voter.id} className="inline-block h-6 w-6 rounded-full ring-2 ring-white" title={voter.name || voter.username || 'User'}>
        ...
        {voters.length > 5 && (<div className="flex items-center justify-center h-6 w-6 rounded-full bg-gray-200 text-gray-600 text-xs font-medium ring-2 ring-white">
            +{voters.length - 5}
          </div>)}
      </div>);
    };
            ...
            <div className="flex justify-between items-start">
              <div>
                ...
                <CardDescription className="mt-1">
                  {item.description}
                  {(item.start_time || item.address) && (<div className="text-xs text-muted-foreground mt-2 flex flex-col sm:flex-row sm:items-center sm:gap-4">
                      {item.start_time && (<span className="flex items-center"><Clock className="h-3 w-3 mr-1"/> {item.start_time}{item.end_time ? ` - ${item.end_time}` : ''}</span>)}
                      {item.address && (<span className="flex items-center mt-1 sm:mt-0"><MapPin className="h-3 w-3 mr-1"/> {item.address}</span>)}
                    </div>)}
                </CardDescription>
              </div>
             ...
             <div className="flex items-center justify-between mt-2 pt-2 border-t">
                ...
                <div className="flex items-center gap-4">
                   <div className="flex items-center gap-1">
                      <Button size="sm" variant={item.votes.userVote === 'up' ? 'default' : 'ghost'} onClick={() => handleVote(item.id, 'up')} className="p-1 h-auto">
                  ...
                  <div className="flex items-center gap-1">
                       <Button size="sm" variant={item.votes.userVote === 'down' ? 'destructive' : 'ghost'} onClick={() => handleVote(item.id, 'down')} className="p-1 h-auto">
