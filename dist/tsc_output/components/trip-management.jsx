...
export function TripManagement({ tripId }) {
    ...
    useEffect(() => {
        async function fetchTripAndMembers() {
            try {
                ...
                setPublicLink(tripData.trip.public_slug ? `${window.location.origin}/trips/public/${tripData.trip.public_slug}` : "");
                ...
                const currentUserMember = data.find((m) => m.id === (user === null || user === void 0 ? void 0 : user.id));
                setIsAdmin((currentUserMember === null || currentUserMember === void 0 ? void 0 : currentUserMember.role) === TRIP_ROLES.ADMIN || (currentUserMember === null || currentUserMember === void 0 ? void 0 : currentUserMember.role) === TRIP_ROLES.EDITOR);
            }
            catch (error) {
        ...
        try {
            ...
            const response = await fetch(API_ROUTES.TRIP_DETAILS(tripId), {
                ...
                body: JSON.stringify({
                    ...
                    public_slug: newStatus && !trip.public_slug
    ...
    const updateMemberRole = async (memberId, newRole) => {
        try {
            ...
            // Update local state
            setMembers(members.map((member) => (member.id === memberId ? Object.assign(Object.assign({}, member), { role: newRole }) : member)));
            toast({
                title: "Role updated",
    ...
    return (<Card className="w-full">
      <CardHeader>
        <div className="flex justify-between items-center">
          <div>
            <CardTitle>{trip === null || trip === void 0 ? void 0 : trip.name}</CardTitle>
            <CardDescription>Trip Management</CardDescription>
          </div>
          {isAdmin && !editMode && (<Button variant="outline" onClick={() => setEditMode(true)}>
          ...
          <TabsContent value="general" className="space-y-4 pt-4">
            {editMode ? (<div className="space-y-4">
                <div>
                  ...
                  <Input id="trip-name" value={editedTrip.name} onChange={(e) => setEditedTrip(Object.assign(Object.assign({}, editedTrip), { name: e.target.value }))}/>
                </div>
                <div>
                  <Label htmlFor="trip-description">Description</Label>
                  <Textarea id="trip-description" value={editedTrip.description || ""} onChange={(e) => setEditedTrip(Object.assign(Object.assign({}, editedTrip), { description: e.target.value }))}/>
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <Label htmlFor="start-date">Start Date</Label>
                    <Input id="start-date" type="date" value={editedTrip.start_date ? new Date(editedTrip.start_date).toISOString().split("T")[0] : ""} onChange={(e) => setEditedTrip(Object.assign(Object.assign({}, editedTrip), { start_date: e.target.value }))}/>
                  </div>
                  <div>
                    <Label htmlFor="end-date">End Date</Label>
                    <Input id="end-date" type="date" value={editedTrip.end_date ? new Date(editedTrip.end_date).toISOString().split("T")[0] : ""} onChange={(e) => setEditedTrip(Object.assign(Object.assign({}, editedTrip), { end_date: e.target.value }))}/>
                  </div>
                </div>
              </div>) : (<div className="space-y-4">
                <div>
                  <h3 className="text-sm font-medium">Trip Name</h3>
                  <p>{trip === null || trip === void 0 ? void 0 : trip.name}</p>
                </div>
                <div>
                  <h3 className="text-sm font-medium">Description</h3>
                  <p>{(trip === null || trip === void 0 ? void 0 : trip.description) || "No description"}</p>
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <h3 className="text-sm font-medium">Start Date</h3>
                    <p>{(trip === null || trip === void 0 ? void 0 : trip.start_date) ? new Date(trip.start_date).toLocaleDateString() : "Not set"}</p>
                  </div>
                  <div>
                    <h3 className="text-sm font-medium">End Date</h3>
                    <p>{(trip === null || trip === void 0 ? void 0 : trip.end_date) ? new Date(trip.end_date).toLocaleDateString() : "Not set"}</p>
                  </div>
                </div>
                <div>
                  <h3 className="text-sm font-medium">Destination</h3>
                  <p>{(trip === null || trip === void 0 ? void 0 : trip.destination) || "No destination set"}</p>
                </div>
              </div>)}
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <div className="space-y-2">
                  {members.map((member) => {
            var _a;
            return (<div key={member.id} className="flex items-center justify-between p-2 border rounded-md">
                      <div className="flex items-center gap-2">
                        <Avatar>
                          <AvatarImage src={member.avatar_url || "/images/placeholder-avatar.png"}/>
                          <AvatarFallback>{((_a = member.name) === null || _a === void 0 ? void 0 : _a.charAt(0)) || member.email.charAt(0)}</AvatarFallback>
                        </Avatar>
                        <div>
                      ...
                      {isAdmin && member.id !== (trip === null || trip === void 0 ? void 0 : trip.owner_id) && (<div className="flex gap-2">
                          <Button size="sm" variant="outline" onClick={() => updateMemberRole(member.id, member.role === "admin" ? "member" : "admin")}>
