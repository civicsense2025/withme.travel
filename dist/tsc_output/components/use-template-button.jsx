...
export function UseTemplateButton({ templateId, className }) {
    const router = useRouter();
    const { toast } = useToast();
    const { trips } = useTrips();
    const [isLoading, setIsLoading] = useState(false);
    const [showTripSelector, setShowTripSelector] = useState(false);
    const handleApplyTemplate = async (tripId) => {
        setIsLoading(true);
        try {
    ...
    if (!(trips === null || trips === void 0 ? void 0 : trips.length)) {
        return (<Button className={className} onClick={() => router.push("/trips/create")}>
        Create a Trip First
      </Button>);
    }
    return (<Dialog open={showTripSelector} onOpenChange={setShowTripSelector}>
      ...
      <DialogContent className="sm:max-w-[425px]">
        ...
        <ScrollArea className="max-h-[300px] mt-4">
          <div className="space-y-4">
            {trips.map((trip) => (<Card key={trip.id} className="cursor-pointer hover:bg-accent transition-colors" onClick={() => handleApplyTemplate(trip.id)}>
                <CardHeader>
