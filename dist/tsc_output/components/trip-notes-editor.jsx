...
import { Bold, Italic, List, ListOrdered, Image as ImageIcon, Link as LinkIcon, Heading1, Heading2, Undo, Redo, Save, Loader2 } from "lucide-react";
import { toast } from "@/hooks/use-toast";
export function TripNotesEditor({ tripId, noteId, initialContent = "", placeholder = "Add notes about your trip...", readOnly = false, }) {
    ...
    return (<div className="border rounded-md space-y-4">
      {!readOnly && (<div className="flex flex-wrap items-center gap-1 p-2 border-b">
          ...
          <Button variant="ghost" size="icon" onClick={() => editor.chain().focus().toggleBulletList().run()} className={editor.isActive('bulletList') ? 'bg-muted' : ''} title="Bullet List">
          ...
          <Button variant="ghost" size="icon" onClick={() => { var _a; return (_a = fileInputRef.current) === null || _a === void 0 ? void 0 : _a.click(); }} disabled={isUploading} title="Upload Image">
          ...
          <Button variant="ghost" size="icon" onClick={saveContent} disabled={isSaving || !noteId} title="Save Note">
            {isSaving ? <Loader2 className="h-4 w-4 animate-spin"/> : <Save className="h-4 w-4"/>}
          </Button>
        </div>)}

      {editor && (<BubbleMenu editor={editor} tippyOptions={{ duration: 100 }} shouldShow={({ editor, view, state, oldState, from, to }) => {
