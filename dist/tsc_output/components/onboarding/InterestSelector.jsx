...
export function InterestSelector({ onComplete, onBack, onSkip, suggestedInterests }) {
    const { toast } = useToast();
    const { getTags, updateUserInterest, isLoading } = useTags();
    const [tags, setTags] = useState([]);
    const [selectedInterests, setSelectedInterests] = useState(suggestedInterests || {});
    ...
    useEffect(() => {
        const loadTags = async () => {
            ...
            if (Object.keys(suggestedInterests).length > 0) {
                ...
                setSelectedInterests(prev => (Object.assign(Object.assign({}, prev), tagSuggestions)));
            }
        };
        loadTags();
    }, [getTags, suggestedInterests]);
    const handleInterestChange = (tagId, value) => {
        setSelectedInterests(prev => (Object.assign(Object.assign({}, prev), { [tagId]: value[0] })));
    };
    const handleComplete = async () => {
        try {
            setIsSaving(true);
            const promises = Object.entries(selectedInterests).map(([tagId, strength]) => updateUserInterest(tagId, strength));
      ...
      <CardContent className="pt-6 pb-8 px-6">
        ...
        <div className="grid gap-4 sm:grid-cols-2 mb-6">
          {sortedTags.map((tag) => {
            ...
            return (<Card key={tag.id} className={`p-4 space-y-4 group hover:shadow-md transition-shadow ${isSuggested ? 'border-primary/20' : ''}`}>
                <div className="flex items-center justify-between">
                  <div>
                    <h3 className="font-medium flex items-center gap-2">
                      {tag.emoji && <span className="group-hover:scale-110 transition-transform">{tag.emoji}</span>}
                      {tag.name}
                      {isSuggested && (<span className="text-xs bg-primary/10 text-primary px-2 py-0.5 rounded-full">
                          Suggested
                        </span>)}
                    </h3>
                ...
                <Slider defaultValue={[selectedInterests[tag.id] || 0]} max={100} step={10} value={[selectedInterests[tag.id] || 0]} onValueChange={(value) => handleInterestChange(tag.id, value)} className="w-full"/>
              </Card>);
        })}
        </div>
        ...
        <div className="flex flex-col gap-3">
          <div className="flex gap-3">
            ...
            <Button onClick={handleComplete} disabled={isSaving || Object.keys(selectedInterests).length === 0} className="flex-1">
