...
export function TravelPersonalityScreen({ userData, onInputChange, onNext, onBack, onSkip }) {
    const personalities = [
        {
            id: "planner",
            emoji: "🗓️",
    ...
    return (<Card className="border-0 shadow-lg">
      <CardContent className="pt-6 pb-8 px-6">
        ...
        <div className="grid grid-cols-2 gap-3 mb-6">
          {personalities.map((personality) => (<Button key={personality.id} variant={userData.travelPersonality === personality.id ? "default" : "outline"} className="h-auto py-4 flex flex-col gap-2 group transition-all" onClick={() => onInputChange("travelPersonality", personality.id)}>
              ...
              <div className="flex flex-wrap gap-1 justify-center">
                {personality.tags.map(tag => (<Badge key={tag} variant="secondary" className="text-[10px] px-1.5 py-0">
                    {tag}
                  </Badge>))}
              </div>
            </Button>))}
