...
export function AdminDestinations() {
    ...
    const filteredDestinations = destinations.filter((destination) => {
        var _a, _b;
        return (((_a = destination.name) === null || _a === void 0 ? void 0 : _a.toLowerCase()) || "").includes(searchTerm.toLowerCase()) ||
            (((_b = destination.country) === null || _b === void 0 ? void 0 : _b.toLowerCase()) || "").includes(searchTerm.toLowerCase());
    });
    ...
    const handleUpdateDestination = async () => {
        ...
        try {
            ...
            if (error)
                throw error;
            // Update UI
            setDestinations(destinations.map((dest) => dest.id === destinationToEdit.id
                ? Object.assign(Object.assign({}, dest), { name: formData.name, country: formData.country, description: formData.description || null, image_url: formData.image_url || null }) : dest));
    ...
    return (<div className="space-y-4">
      ...
      {loading ? (<div className="flex justify-center items-center h-64">
          ...
          <Table>
            ...
            <TableBody>
              {filteredDestinations.length === 0 ? (<TableRow>
                  <TableCell colSpan={5} className="text-center py-8 text-muted-foreground">
                    No destinations found
                  </TableCell>
                </TableRow>) : (filteredDestinations.map((destination) => (<TableRow key={destination.id}>
                    <TableCell className="font-medium">{destination.name}</TableCell>
                    <TableCell>{destination.country}</TableCell>
                    <TableCell className="max-w-xs truncate">{destination.description || "No description"}</TableCell>
                    <TableCell>
                      {destination.image_url ? (<div className="h-10 w-10 rounded overflow-hidden bg-muted">
                          <img src={destination.image_url || "/placeholder.svg"} alt={destination.name} className="h-full w-full object-cover" onError={(e) => {
                        e.currentTarget.src = "/destinations/paris-eiffel-tower.png";
                    }}/>
                        </div>) : (<div className="h-10 w-10 rounded flex items-center justify-center bg-muted">
        ...
        <DialogContent>
          ...
          <div className="space-y-4 py-4">
            <div className="space-y-2">
              ...
              <Input id="name" name="name" value={formData.name} onChange={handleInputChange} placeholder="e.g. Paris"/>
            </div>
            <div className="space-y-2">
              <Label htmlFor="country">Country</Label>
              <Input id="country" name="country" value={formData.country} onChange={handleInputChange} placeholder="e.g. France"/>
            </div>
            <div className="space-y-2">
              <Label htmlFor="description">Description</Label>
              <Textarea id="description" name="description" value={formData.description} onChange={handleInputChange} placeholder="Enter a brief description of the destination" rows={3}/>
            </div>
            <div className="space-y-2">
              <Label htmlFor="image_url">Image URL</Label>
              <Input id="image_url" name="image_url" value={formData.image_url} onChange={handleInputChange} placeholder="https://example.com/image.jpg"/>
            </div>
          </div>
          <DialogFooter>
      ...
      <Dialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Are you sure you want to delete the destination "{destinationToDelete === null || destinationToDelete === void 0 ? void 0 : destinationToDelete.name}"? This action cannot be
              undone.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
