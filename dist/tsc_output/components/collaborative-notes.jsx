...
export function CollaborativeNotes({ tripId, readOnly = false, }) {
    ...
    const saveTagsTimeoutRef = useRef(null);
    ...
    useEffect(() => {
        async function fetchNoteDetails() {
            ...
            try {
                ...
                const noteContentPromise = fetch(`${API_ROUTES.COLLABORATIVE_NOTES(tripId)}/${selectedNoteId}`);
                const noteTagsPromise = fetch(`${API_ROUTES.COLLABORATIVE_NOTES(tripId)}/${selectedNoteId}/tags`);
                const [contentResponse, tagsResponse] = await Promise.all([noteContentPromise, noteTagsPromise]);
                // Handle content response
                if (!contentResponse.ok) {
                    if (contentResponse.status === 404) {
                        toast({ title: "Note not found", description: "Maybe it was deleted? Refreshing list...", variant: "destructive" });
                ...
                if (tagsResponse.ok) {
                    ...
                    const fetchedTags = (tagsData.tags || []).map((tag) => ({ id: tag.id, name: tag.name }));
    ...
    const handleCreateNote = async () => {
        if (!newNoteTitle.trim()) {
            toast({ title: "Title required", description: "Please enter a title for your new note.", variant: "destructive" });
            return;
        }
        ...
        try {
            ...
            toast({ title: "Error Creating Note", description: formatError(err), variant: "destructive" });
        }
        finally {
            setIsLoadingNote(false);
        }
    };
    const confirmDeleteNote = async (noteIdToDelete) => {
        if (!noteIdToDelete)
            return;
        setIsLoadingNote(true);
        try {
            ...
            toast({ title: "Error Deleting Note", description: formatError(err), variant: "destructive" });
        }
        finally {
            setIsLoadingNote(false);
        }
    ...
    const handleTagsChange = (newTagNames) => {
        if (readOnly || !selectedNoteId)
            return; // Prevent changes if readOnly or no note selected
    ...
    const saveTags = async (tagNamesToSave, previousTags) => {
        if (!selectedNoteId)
            return;
        setIsSavingTags(true);
        console.log("Saving tags:", tagNamesToSave); // Debug log
        try {
            const response = await fetch(`${API_ROUTES.COLLABORATIVE_NOTES(tripId)}/${selectedNoteId}/tags`, {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
            ...
            const finalTags = (data.tags || []).map((tag) => ({ id: tag.id, name: tag.name }));
    ...
    return (<div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-8">
      {/* Notes List Column */}
      <div className="md:col-span-1 space-y-4">
        <Card className="overflow-hidden">
          <CardHeader className="p-4 border-b flex flex-row items-center justify-between">
            ...
            {!readOnly && (<div>
                {isCreating ? (<div className="flex items-center gap-1">
                    <Input placeholder="New title..." value={newNoteTitle} onChange={(e) => setNewNoteTitle(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleCreateNote()} autoFocus className="h-8 w-32"/>
                    <Button onClick={handleCreateNote} size="icon" variant="ghost" disabled={isLoadingNote || !newNoteTitle.trim()} className="h-8 w-8">
                      {isLoadingNote ? <Loader2 className="h-4 w-4 animate-spin"/> : <Plus className="h-4 w-4"/>}
                      <span className="sr-only">Create Note</span>
                    </Button>
                     <Button onClick={() => setIsCreating(false)} size="icon" variant="ghost" disabled={isLoadingNote} className="h-8 w-8">
                        <X className="h-4 w-4"/>
                       <span className="sr-only">Cancel</span>
                    </Button>
                  </div>) : (<Button size="sm" variant="ghost" onClick={() => setIsCreating(true)} className="gap-1">
          ...
          <CardContent className="p-0">
            ...
            {/* Notes List */}
            {isLoadingList ? (<div className="space-y-2 p-4">
                <Skeleton className="h-6 w-3/4"/>
                <Skeleton className="h-6 w-1/2"/>
                <Skeleton className="h-6 w-2/3"/>
              </div>) : error ? (<div className="p-4 text-center text-destructive">{error}</div>) : notesList.length === 0 ? (<div className="p-4 text-center text-muted-foreground italic">
                {readOnly ? 'No notes yet.' : 'Create your first note!'}
              </div>) : (<ScrollArea className="h-[calc(100vh-20rem)]">
                <div className="divide-y divide-border">
                  {notesList.map((note) => (<div key={note.id} className="flex items-center justify-between w-full text-left px-4 py-2 group">
                      <button onClick={() => setSelectedNoteId(note.id)} disabled={isLoadingNote} className={`flex-1 truncate pr-2 text-sm hover:text-primary transition-colors disabled:opacity-50 ${selectedNoteId === note.id ? "font-semibold text-primary" : ""}`}>
                       {note.title}
                      </button>
                       {!readOnly && (<AlertDialog>
                           <AlertDialogTrigger asChild>
                             <Button variant="ghost" size="icon" className="h-6 w-6 text-muted-foreground hover:text-destructive opacity-0 group-hover:opacity-100 transition-opacity flex-shrink-0" aria-label="Delete note">
      ...
      <div className="md:col-span-2">
        {isLoadingNote && selectedNoteId ? (<Card className="min-h-[400px]">
            <CardHeader><Skeleton className="h-6 w-1/2"/></CardHeader>
            <CardContent className="space-y-4 pt-4">
               <Skeleton className="h-4 w-1/4"/>
               <Skeleton className="h-64 w-full"/>
            </CardContent>
          </Card>) : selectedNote ? (<Card className="overflow-hidden">
            ...
            <CardContent className="p-4">
              ...
              {!readOnly && (<div className="mb-4">
                  <Label className="mb-2 block text-sm font-medium">Tags</Label>
                   <TagInput value={currentNoteTags.map(tag => tag.name)} onChange={handleTagsChange} placeholder="Add relevant tags..."/>
                   {isSavingTags && <p className="text-xs text-muted-foreground mt-1 animate-pulse">Saving tags...</p>}
                 </div>)}
              {/* Editor */}
              <TripNotesEditor noteId={selectedNote.id} initialContent={selectedNote.content} tripId={tripId} readOnly={readOnly}/>
            </CardContent>
          </Card>) : (<div className="flex items-center justify-center h-[400px] border rounded-lg bg-muted/20">
            <p className="text-muted-foreground italic">
              {notesList.length > 0 ? "Select a note to view/edit" : (readOnly ? "No notes to display" : "Create a note to get started")}
            </p>
          </div>)}
      </div>
    </div>);
