...
function BubbleItem({ bubble, isInView }) {
    const index = bubble.id;
    return (<Link key={bubble.id} href={`/destinations/${bubble.citySlug}`} passHref>
      <motion.div className={`
        ...
        `} style={{
            left: bubble.x,
            top: bubble.y,
        }} initial={{ scale: 0, opacity: 0 }} animate={isInView ?
            {
                scale: 1,
                opacity: 0.95,
                transition: {
...
export function CityBubbles() {
    ...
    return (<div ref={bubblesRef} className="relative w-full mx-auto overflow-hidden py-24 md:py-20">
