    ...
    return (<div className="container py-8">
      ...
      <p className="text-muted-foreground mb-6">
        {isLoading ? (<span className="flex items-center">
            ...
            Searching...
          </span>) : (`Found ${totalResults} results for "${query}"`)}
      </p>
      ...
      <Tabs defaultValue="all" value={activeTab} onValueChange={setActiveTab} className="mb-6">
        ...
        <TabsContent value="all" className="mt-6">
          {destinations.length > 0 && (<div className="mb-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {destinations.slice(0, 3).map((destination) => (<DestinationCard key={destination.id} destination={destination}/>))}
              </div>
              {destinations.length > 3 && (<div className="mt-4 text-center">
                  <Link href="#" onClick={(e) => {
                    e.preventDefault();
          ...
          {trips.length > 0 && (<div>
              ...
              {trips.length > 3 && (<div className="mt-4 text-center">
                  <Link href="#" onClick={(e) => {
                    e.preventDefault();
                    setActiveTab("trips");
                }} className="text-primary hover:underline">
          ...
          {totalResults === 0 && !isLoading && (<div className="text-center py-12">
        ...
        <TabsContent value="destinations" className="mt-6">
          ...
          {destinations.length === 0 && !isLoading && (<div className="text-center py-12">
        ...
        <TabsContent value="trips" className="mt-6">
          ...
          {trips.length === 0 && !isLoading && (<div className="text-center py-12">
...
function DestinationCard({ destination }) {
    ...
    return (<Link href={`/destinations/${destination.city.toLowerCase().replace(/\s+/g, "-")}`}>
...
function TripCard({ trip }) {
    return (<Link href={`/trips/${trip.id}`}>
      <Card className="overflow-hidden hover:shadow-md transition-shadow h-full">
        <div className="relative h-40 w-full">
          <Image src={trip.image_url ||
            `/placeholder.svg?height=400&width=600&query=${encodeURIComponent(trip.destination) || "/placeholder.svg"}`} alt={trip.title} fill className="object-cover"/>
        </div>
