...
export async function generateMetadata({ params }) {
    ...
    return {
        ...
        description: itinerary.description || `View the ${itinerary.duration_days}-day itinerary template for ${((_a = itinerary.destinations) === null || _a === void 0 ? void 0 : _a.city) || 'a destination'}.`,
    };
}
...
export default async function ItineraryPage({ params }) {
    ...
    const imageUrl = itinerary.image_url || ((_a = itinerary.destinations) === null || _a === void 0 ? void 0 : _a.image_url) || "/placeholder.svg";
    const authorName = ((_b = itinerary.users) === null || _b === void 0 ? void 0 : _b.full_name) || "Anonymous";
    const authorAvatar = ((_c = itinerary.users) === null || _c === void 0 ? void 0 : _c.avatar_url) || "/images/placeholder-avatar.png";
    return (<div className="container py-10">
      {/* Header Section */}
      <div className="relative mb-8 h-64 md:h-96 w-full overflow-hidden rounded-lg">
        <Image src={imageUrl} alt={itinerary.title} fill priority className="object-cover"/>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <Card>
            <CardHeader>
              ...
              <div className="flex flex-wrap gap-2 pt-2">
                {(_d = itinerary.tags) === null || _d === void 0 ? void 0 : _d.map((tag) => (<Badge key={tag} variant="outline">{tag}</Badge>))}
              </div>
            </CardHeader>
          </Card>
            ...
            <CardContent>
              <Tabs defaultValue="day-1">
                <TabsList className="mb-4 flex flex-wrap h-auto justify-start">
                  {Array.isArray(itinerary.days) && itinerary.days.length > 0 ? (itinerary.days.map((dayData, index) => (<TabsTrigger key={`day-${dayData.day || index + 1}`} value={`day-${dayData.day || index + 1}`} className="lowercase">
                        Day {dayData.day || index + 1}
                      </TabsTrigger>))) : (<TabsTrigger value="day-1" disabled>No Plan Available</TabsTrigger>)}
                </TabsList>
                
                {Array.isArray(itinerary.days) && itinerary.days.length > 0 ? (itinerary.days.map((dayData, index) => (<TabsContent key={`content-day-${dayData.day || index + 1}`} value={`day-${dayData.day || index + 1}`} className="space-y-4">
                      <h3 className="font-semibold text-lg">{dayData.title || `Day ${dayData.day || index + 1}`}</h3>
                      {Array.isArray(dayData.activities) && dayData.activities.length > 0 ? (dayData.activities.map((activity, actIndex) => (<div key={actIndex} className="flex gap-4 items-start">
                            ...
                            </div>))) : (<p className="text-muted-foreground italic">No activities planned for this day.</p>)}
                    </TabsContent>))) : (<TabsContent value="day-1"> 
        ...
        <div className="lg:col-span-1 space-y-6">
          <Card className="bg-gradient-to-br from-travel-blue/10 to-travel-purple/10">
            ...
            <CardContent>
              <UseTemplateButton templateId={itinerary.id} className="w-full lowercase bg-travel-purple hover:bg-purple-400 text-purple-900"/>
            </CardContent>
          </Card>
