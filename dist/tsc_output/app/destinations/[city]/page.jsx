...
import { ArrowLeft, Calendar, Globe, DollarSign, Utensils, Camera, Moon, Sun, MapPin, Wifi, Train, Leaf, FootprintsIcon as Walking, Heart, Instagram, Briefcase, Shield, Accessibility, Users, Info, PlusCircle, } from "lucide-react";
...
export default function CityPage() {
    ...
    const cityParam = params === null || params === void 0 ? void 0 : params.city;
    useEffect(() => {
        if (!cityParam) {
    ...
    const getDestinationImageData = (destination) => {
        var _a, _b;
        if (!destination) {
            return {
                url: "/placeholder.svg",
        ...
        return {
            url: imageUrl,
            alt: ((_a = destination.image_metadata) === null || _a === void 0 ? void 0 : _a.alt_text) || `${destination.city}, ${destination.country}`,
            attribution: (_b = destination.image_metadata) === null || _b === void 0 ? void 0 : _b.attribution
        };
    };
    if (!cityParam) {
    ...
    const defaultTripName = `${(profile === null || profile === void 0 ? void 0 : profile.first_name) || 'My'}'s trip to ${destination.city}`;
    return (<div className="mx-auto max-w-6xl px-4 py-6">
      <div className="flex items-center mb-6">
        <Link href="/destinations">
          <Button variant="ghost" size="sm" className="gap-1">
      ...
      <div className="relative h-64 md:h-96 w-full rounded-lg overflow-hidden group transition-transform duration-300 hover:scale-[1.02]">
        <Image src={imageData.url} alt={imageData.alt} fill priority className="object-cover transition-transform duration-700 group-hover:scale-105"/>
        {imageData.attribution && (<div className="absolute bottom-2 right-2 z-10">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-12">
        <div className="md:col-span-2 space-y-8">
          <Card className="group transition-all duration-300 hover:shadow-lg">
            <CardContent className="pt-6">
              ...
              {destination.description && (<div className="text-muted-foreground prose prose-sm dark:prose-invert [&>p]:my-4" dangerouslySetInnerHTML={{
                    ...
                    : destination.description.split('\n\n').map(block => {
                        ...
                        if (block.includes('\n') && !block.trim().endsWith('.')) {
                            ...
                            if (first.trim().endsWith(':')) {
                                return `<p>${first}</p><ul>${rest.map(item => `<li>${item.trim()}</li>`).join('')}</ul>`;
                            }
                            // Otherwise treat all lines as list items
                            return `<ul>${lines.map(item => `<li>${item.trim()}</li>`).join('')}</ul>`;
                        }
                        return `<p>${block}</p>`;
                    }).join('')
            }}/>)}

              {destination.highlights && (<div className="mt-6">
                  <h3 className="text-lg font-semibold mb-2 lowercase">highlights</h3>
                  <div className="prose prose-sm dark:prose-invert [&>p]:my-4" dangerouslySetInnerHTML={{
              ...
              <div className="mt-6 flex flex-wrap gap-2">
                {destination.family_friendly && (<Badge variant="outline" className="transition-all duration-300 hover:scale-110">Family Friendly</Badge>)}
                {destination.digital_nomad_friendly >= 4 && (<Badge variant="outline" className="transition-all duration-300 hover:scale-110">Digital Nomad Friendly</Badge>)}
                {destination.beach_quality !== null && destination.beach_quality >= 4 && (<Badge variant="outline" className="transition-all duration-300 hover:scale-110">Great Beaches</Badge>)}
                {destination.cultural_attractions >= 4 && (<Badge variant="outline" className="transition-all duration-300 hover:scale-110">Cultural Hotspot</Badge>)}
                {destination.nightlife_rating >= 4 && (<Badge variant="outline" className="transition-all duration-300 hover:scale-110">Vibrant Nightlife</Badge>)}
                {destination.outdoor_activities >= 4 && (<Badge variant="outline" className="transition-all duration-300 hover:scale-110">Outdoor Activities</Badge>)}
                {destination.lgbtq_friendliness >= 4 && (<Badge variant="outline" className="transition-all duration-300 hover:scale-110">LGBTQ+ Friendly</Badge>)}
                {destination.accessibility >= 4 && (<Badge variant="outline" className="transition-all duration-300 hover:scale-110">Accessible</Badge>)}
              </div>
            </CardContent>
          </Card>
        ...
        <div className="space-y-8">
          <Card className="group transition-all duration-300 hover:shadow-lg">
            ...
            router.push(`/trips/create?destination_id=${destination.id}&trip_name=${encodeURIComponent(defaultTripName)}`);
              ...
              {destination.tourism_website && (<div className="mt-8">
                  <a href={destination.tourism_website} target="_blank" rel="noopener noreferrer" className="text-sm text-blue-600 hover:underline flex items-center gap-1.5 transition-all duration-300 hover:translate-x-1">
        ...
        ].map((item, index) => (<div key={index} className="flex justify-between items-center p-2 rounded-lg transition-all duration-300 hover:bg-muted/50">
