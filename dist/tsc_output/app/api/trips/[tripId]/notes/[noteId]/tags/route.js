...
async function checkTripMembershipAndRole(supabase, tripId, userId, roles) {
    ...
    if (error) {
        console.error(`Error checking trip membership/role for user ${userId} on trip ${tripId}:`, error);
...
export async function PUT(request, props) {
    ...
    try {
        const supabase = createClient();
        const { tags: newTags } = await request.json();
        // Auth check
        const { data: { user }, error: authError } = await supabase.auth.getUser();
        if (authError || !user) {
            return NextResponse.json({ error: "Authentication required" }, { status: 401 });
        ...
        if (!canEdit) {
            return NextResponse.json({ error: "Forbidden: You don't have permission to manage tags for this note" }, { status: 403 });
        }
        // Validate input: ensure tagNames is an array of strings
        if (!Array.isArray(newTags) || !newTags.every(t => typeof t === 'string')) {
            return NextResponse.json({ error: "Invalid input: 'tags' must be an array of strings." }, { status: 400 });
        }
        ...
        if (newTagNames.length > 0) {
            ...
            insertedTags.forEach(tag => allTagIds.push(tag.id));
        }
        ...
        if (deleteError) {
            ...
            return NextResponse.json({ error: "Error updating note tags (delete step)" }, { status: 500 });
        }
        ...
        if (allTagIds.length > 0) {
            ...
            if (insertNoteTagsError) {
                console.error(`[Note Tags API PUT ${noteId}] Error inserting new note tags:`, insertNoteTagsError);
                return NextResponse.json({ error: "Error updating note tags (insert step)" }, { status: 500 });
            }
        }
        ...
        const finalTags = finalTagsData ? finalTagsData.map(item => item.tags).filter(tag => tag !== null) : [];
