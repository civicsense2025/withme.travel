...
export async function GET(request) {
    try {
        ...
        if (userError || !user) {
            console.error("API Likes GET Error: Unauthorized - ", userError === null || userError === void 0 ? void 0 : userError.message);
...
export async function POST(request) {
    try {
        ...
        const { data: { user }, error: userError } = await supabase.auth.getUser();
        if (userError || !user) {
            console.error("API Likes POST Error: Unauthorized - ", userError === null || userError === void 0 ? void 0 : userError.message);
            return NextResponse.json({ error: 'Unauthorized' }, { status: 401 });
        }
        // Get request body
...
export async function DELETE(request) {
    try {
        ...
        const { data: { user }, error: userError } = await supabase.auth.getUser();
        if (userError || !user) {
            console.error("API Likes DELETE Error: Unauthorized - ", userError === null || userError === void 0 ? void 0 : userError.message);
            return NextResponse.json({ error: 'Unauthorized' }, { status: 401 });
        }
        // Get query params
        const { searchParams } = new URL(request.url);
