...
import { DB_TABLES } from "@/utils/constants"; // Keep DB_TABLES import
export async function GET(request) {
    try {
        ...
        if (error) {
            ...
            return new NextResponse(`Failed to fetch places: ${error.message || 'Unknown database error'}`, { status: 500 });
        }
        // Process the places data directly from the places table columns
        const processedPlaces = places.map((place) => (Object.assign(Object.assign({}, place), { rating: place.rating || 0, rating_count: place.rating_count || 0 })));
...
export async function POST(request) {
    try {
        ...
        if (!name || !destination_id || !category) {
            return NextResponse.json({ error: "Missing required fields: name, destination_id, and category are required." }, { status: 400 });
        }
        ...
        if (insertError) {
            ...
            return NextResponse.json({ error: insertError.message || "Failed to save suggestion." }, { status: 500 });
        }
