...
import { getGroup, SplitwiseError } from "@/lib/services/splitwise";
...
export async function GET(request) {
    try {
        ...
        if (tripDataError || !trip || !trip.splitwise_group_id) {
            return NextResponse.json({ error: "Trip not found or not linked to Splitwise" }, { status: 404 });
        }
        ...
        const candidates = splitwiseMembers.map(swMember => {
            ...
            if (existingProfile && currentTripMemberIds.has(existingProfile.id)) {
                return Object.assign(Object.assign({}, swMember), { status: "member", profileId: existingProfile.id });
            }
            else if (existingProfile) {
                return Object.assign(Object.assign({}, swMember), { status: "existing_user", profileId: existingProfile.id });
            }
            else {
                return Object.assign(Object.assign({}, swMember), { status: "new_user" });
            }
        });
        ...
        return NextResponse.json({ error: error.message || "Failed to fetch Splitwise import candidates" }, { status: 500 });
    }
}
