...
export default function ContinentsPage() {
    return (<div className="container py-10">
      <PageHeader heading="Explore Continents" description="Discover destinations across the world organized by continent"/>
      
      <motion.div className="grid grid-cols-1 md:grid-cols-2 gap-8 mt-10" variants={containerVariants} initial="hidden" animate="visible">
        {CONTINENTS.map((continent) => (<motion.div key={continent.slug} variants={itemVariants} transition={{ duration: 0.5 }} whileHover={{
                scale: 1.03,
            ...
            <Link href={`/continents/${continent.slug}`} className="block h-full">
              <div className="relative rounded-xl overflow-hidden h-64 group">
                ...
                <Image src={continent.coverImage} alt={continent.name} fill className="object-cover transition-transform duration-500 group-hover:scale-105"/>
