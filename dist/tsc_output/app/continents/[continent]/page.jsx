...
export default function ContinentPage() {
    ...
    const continentData = CONTINENT_DATA[continentSlug] || {
    ...
    return (<div className="relative">
      ...
      <div className="relative h-[50vh] min-h-[400px] w-full">
        ...
        <Image src={continentData.coverImage} alt={`${continentData.name} continent`} fill className="object-cover" priority/>
        <div className="absolute inset-0 z-20 flex items-center justify-center">
          <div className="container text-center text-white">
            <motion.h1 className={`text-5xl md:text-6xl font-bold mb-4 text-${continentData.accentColor}`} initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
              {continentData.name}
            </motion.h1>
            <motion.p className="text-xl max-w-3xl mx-auto" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.5, delay: 0.2 }}>
        ...
        <Tabs defaultValue="overview" className="w-full" onValueChange={setActiveTab}>
          ...
          <TabsContent value="overview">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              {/* Stats section */}
              <motion.div className="col-span-1 bg-muted/30 p-6 rounded-xl" variants={containerVariants} initial="hidden" animate="visible">
              ...
              {/* Highlights section */}
              <motion.div className="col-span-2 bg-muted/30 p-6 rounded-xl" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.3 }}>
                <h2 className="text-xl font-semibold mb-6 lowercase">Highlights</h2>
                <ul className="space-y-3">
                  {continentData.highlights.map((highlight, index) => (<motion.li key={index} className="flex items-start gap-2" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.5, delay: 0.1 * (index + 1) }}>
              ...
              <motion.div className="col-span-1 md:col-span-3 bg-muted/30 p-6 rounded-xl mt-6" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.4 }}>
            ...
            <div className="mt-12">
              <div className="flex justify-between items-center mb-6">
                ...
                <Button variant="link" onClick={() => setActiveTab("destinations")} className="lowercase">
              ...
              {isLoading ? (<div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                </div>) : topDestinations.length > 0 ? (<motion.div className="grid grid-cols-1 md:grid-cols-3 gap-6" variants={containerVariants} initial="hidden" animate="visible">
                  {topDestinations.slice(0, 3).map((destination) => (<motion.div key={destination.id} variants={itemVariants}>
                      <DestinationCard destination={destination}/>
                    </motion.div>))}
                </motion.div>) : (<div className="text-center py-10 bg-muted/30 rounded-lg">
          ...
          <TabsContent value="destinations">
            <div className="space-y-8">
              <PageHeader heading={`Destinations in ${continentData.name}`} description={`Explore the diverse destinations across ${continentData.name}`}/>
              
              {isLoading ? (<div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  {[...Array(9)].map((_, i) => (<Skeleton key={i} className="h-64 rounded-xl"/>))}
                </div>) : destinations.length > 0 ? (<>
                  ...
                  <motion.div className="grid grid-cols-1 md:grid-cols-3 gap-6" variants={containerVariants} initial="hidden" animate="visible">
                    {topDestinations.map((destination) => (<motion.div key={destination.id} variants={itemVariants}>
                        <DestinationCard destination={destination}/>
                      </motion.div>))}
                  </motion.div>
                  ...
                  {otherDestinations.length > 0 && (<>
                      ...
                      <motion.div className="grid grid-cols-1 md:grid-cols-3 gap-6" variants={containerVariants} initial="hidden" animate="visible">
                        {otherDestinations.map((destination) => (<motion.div key={destination.id} variants={itemVariants}>
                            <DestinationCard destination={destination}/>
                          </motion.div>))}
                      </motion.div>
                    </>)}
          ...
          <TabsContent value="trips">
            <div className="space-y-8">
              <PageHeader heading={`Popular Trips in ${continentData.name}`} description={`Discover trending itineraries and travel experiences in ${continentData.name}`}/>
