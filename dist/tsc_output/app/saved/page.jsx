...
import { MapPin, Bookmark, FileText, Loader2 } from "lucide-react";
...
export default function SavedPage() {
    ...
    return (<div className="container py-8">
      <PageHeader heading="saved items" description="all your favorite destinations, itineraries, and attractions in one place"/>

      {isLoading || isLoadingDetails ? (<div className="flex justify-center items-center py-16">
          <Loader2 className="h-8 w-8 animate-spin text-primary"/>
        </div>) : totalSaved === 0 ? (<div className="text-center py-16">
        ...
        </div>) : (<Tabs defaultValue="all" value={activeTab} onValueChange={setActiveTab} className="mt-8">
          <TabsList>
          ...
          <TabsContent value="all" className="mt-6">
            {destinations.length > 0 && (<div className="mb-8">
                <h2 className="text-xl font-semibold mb-4">Destinations</h2>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {destinations.map((destination) => (<DestinationCard key={destination.id} destination={destination} onUnlike={() => refetch()}/>))}
                </div>
              </div>)}

            {itineraries.length > 0 && (<div className="mb-8">
                <h2 className="text-xl font-semibold mb-4">Itineraries</h2>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {itineraries.map((itinerary) => (<ItineraryCard key={itinerary.id} itinerary={itinerary} onUnlike={() => refetch()}/>))}
                </div>
              </div>)}

            {attractions.length > 0 && (<div>
                <h2 className="text-xl font-semibold mb-4">Attractions</h2>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {attractions.map((attraction) => (<AttractionCard key={attraction.id} attraction={attraction} onUnlike={() => refetch()}/>))}
                </div>
              </div>)}
          </TabsContent>
          ...
          <TabsContent value="destinations" className="mt-6">
            ...
            {destinations.length === 0 && (<div className="text-center py-12">
          ...
          <TabsContent value="itineraries" className="mt-6">
            ...
            {itineraries.length === 0 && (<div className="text-center py-12">
          ...
          <TabsContent value="attractions" className="mt-6">
            ...
            {attractions.length === 0 && (<div className="text-center py-12">
...
function DestinationCard({ destination, onUnlike }) {
    ...
    const tags = getTags();
    return (<Link href={`/destinations/${destination.city ? destination.city.toLowerCase().replace(/\s+/g, "-") : `destination-${destination.id}`}`}>
      <div className="rounded-3xl overflow-hidden bg-white dark:bg-gray-900 shadow-md hover:shadow-lg transition-all duration-300 flex flex-col h-full border">
        <div className="relative h-48 w-full bg-travel-purple/10">
          <div className="absolute top-3 right-3 z-10">
            <LikeButton itemId={destination.id} itemType="destination" size="sm" className="shadow-sm" onClick={(isLiked) => {
            if (!isLiked && onUnlike)
                onUnlike();
        }}/>
          </div>
          <Image src={destination.image_url ||
            `/placeholder.svg?height=400&width=600&query=${encodeURIComponent(destination.name || destination.city)}`} alt={destination.name || destination.city} fill className="object-cover"/>
        </div>
        <div className="p-5 flex flex-col flex-grow">
          ...
          <div className="flex flex-wrap gap-2 mt-3">
            {tags.map(tag => (<span key={tag} className={`text-sm px-4 py-1.5 rounded-full ${tag === 'food' ? 'bg-travel-mint/80 text-travel-mint-foreground' :
          ...
          {destination.description && (<p className="text-sm text-muted-foreground mt-3 line-clamp-2">{destination.description}</p>)}
        </div>
      </div>
    </Link>);
}
...
function ItineraryCard({ itinerary, onUnlike }) {
    return (<Card className="overflow-hidden">
      <div className="relative h-40 w-full bg-travel-blue/20">
        <div className="absolute top-3 right-3 z-10">
          <LikeButton itemId={itinerary.id} itemType="itinerary" size="sm" className="shadow-sm" onClick={(isLiked) => {
            if (!isLiked && onUnlike)
                onUnlike();
        }}/>
        </div>
...
function AttractionCard({ attraction, onUnlike }) {
    return (<Card className="overflow-hidden">
      <div className="relative h-40 w-full bg-travel-yellow/20">
        <div className="absolute top-3 right-3 z-10">
          <LikeButton itemId={attraction.id} itemType="attraction" size="sm" className="shadow-sm" onClick={(isLiked) => {
            if (!isLiked && onUnlike)
                onUnlike();
        }}/>
        </div>
