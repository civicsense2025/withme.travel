...
const PERSONALITY_SUGGESTED_INTERESTS = {
    planner: {
        "organized-travel": 90,
...
export default function OnboardingPage() {
    ...
    const handleNext = () => {
        ...
        if (step === 3 && userData.travelPersonality) {
            ...
            setUserData(prev => (Object.assign(Object.assign({}, prev), { suggestedInterests })));
        }
        setStep(step + 1);
    };
    const handleBack = () => {
    ...
    const handleSkip = () => {
        ...
        if (step === 3) { // Travel Squad
            setStep(4);
        }
        else if (step === 4) { // Interests
            setStep(5);
        }
    };
    ...
    const handleNextTour = () => {
        if (userData.tourStep < 3) {
            setUserData(Object.assign(Object.assign({}, userData), { tourStep: userData.tourStep + 1 }));
        }
        else {
            router.push("/trips/new");
        }
    ...
    return (<div className="min-h-screen bg-gradient-1 dark:bg-gradient-to-r dark:from-gray-900 dark:to-gray-800 flex items-center justify-center p-4">
      <div className="w-full max-w-md space-y-4">
        <Progress value={progress} className="w-full"/>
        
        {step === 1 && <WelcomeScreen onNext={handleNext}/>}
        
        {step === 2 && (<BasicInfoScreen userData={userData} onInputChange={handleInputChange} onNext={handleNext} onBack={handleBack}/>)}
        
        {step === 3 && (<TravelPersonalityScreen userData={userData} onInputChange={handleInputChange} onNext={handleNext} onBack={handleBack} onSkip={handleSkip}/>)}
        
        {step === 4 && (<TravelSquadScreen userData={userData} onInputChange={handleInputChange} onNext={handleNext} onBack={handleBack} onSkip={handleSkip}/>)}

        {step === 5 && (<InterestSelector onBack={handleBack} onSkip={handleSkip} onComplete={handleNext} suggestedInterests={userData.suggestedInterests}/>)}
        
        {step === 6 && (<SuccessScreen onCreateTrip={handleCreateTrip} onExplore={handleExplore}/>)}
        
        {step === 7 && (<AppTourScreen tourStep={userData.tourStep} onNext={handleNextTour} onSkip={handleSkipTour}/>)}
      </div>
    </div>);
}
