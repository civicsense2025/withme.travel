...
import { DB_TABLES, DB_FIELDS, TRIP_ROLES, PAGE_ROUTES, VOTE_TYPES } from "@/utils/constants";
...
async function getItineraryItems(tripId, userId) {
    ...
    const processedItems = (items === null || items === void 0 ? void 0 : items.map((item) => {
        var _a;
        const currentVotes = item.votes || [];
        // Added type annotations for v
        const upVotes = currentVotes.filter((v) => v.vote_type === VOTE_TYPES.UP);
        const downVotes = currentVotes.filter((v) => v.vote_type === VOTE_TYPES.DOWN);
        // Use provided userId
        const userVote = ((_a = currentVotes.find((v) => { var _a; return ((_a = v.profiles) === null || _a === void 0 ? void 0 : _a.id) === userId; })) === null || _a === void 0 ? void 0 : _a.vote_type) || null;
        // Explicitly type the return object to match DisplayItineraryItem structure
        const displayItem = Object.assign(Object.assign({}, item), { votes: {
                up: upVotes.length,
                ...
                upVoters: upVotes.map((v) => v.profiles).filter((p) => p !== null),
                downVoters: downVotes.map((v) => v.profiles).filter((p) => p !== null),
                userVote: userVote,
            } });
        return displayItem;
    })) || [];
...
async function getSplitwiseDataServer(tripId) {
    ...
    try {
        ...
        if (!response.ok) {
            ...
            // Specifically handle 400 Bad Request (likely not linked)
                ...
                console.warn(`Splitwise API route returned 400 for trip ${tripId}:`, (errorData === null || errorData === void 0 ? void 0 : errorData.error) || '(No specific error message)');
                return { expenses: [], groupName: null }; // Return default for 400 errors
            }
            // Handle other non-OK statuses
            else {
                const errorData = await response.json().catch(() => ({}));
                console.error(`Error fetching Splitwise data server-side (${response.status}):`, errorData);
                return { expenses: [], groupName: null };
            }
        }
...
export default async function TripPage(props) {
    ...
    try {
      ...
      `)
        ...
        trip = tripData;
        destinationId = (_c = (_b = (_a = trip.destinations) === null || _a === void 0 ? void 0 : _a[0]) === null || _b === void 0 ? void 0 : _b.id) !== null && _c !== void 0 ? _c : null;
    }
    catch (error) {
    ...
    let itineraryItemsResult = [];
    let splitwiseDataResult = { expenses: [], groupName: null };
    // Add state for manual expenses
    let manualExpensesResult = [];
    try {
        ...
        if (results[4].status === 'fulfilled') {
            ...
            const manualExpensesData = results[4].value;
            if (manualExpensesData.error) {
                console.error("Failed to fetch manual expenses:", manualExpensesData.error);
            }
            else {
                manualExpensesResult = (_d = manualExpensesData.data) !== null && _d !== void 0 ? _d : [];
            }
        }
        else {
    ...
    const userRole = (_f = (_e = membershipResult === null || membershipResult === void 0 ? void 0 : membershipResult.data) === null || _e === void 0 ? void 0 : _e.role) !== null && _f !== void 0 ? _f : null;
    ...
    return (<TripPageClient tripId={tripId} tripName={trip.name} destinationId={destinationId} initialMembers={(_g = membersResult === null || membersResult === void 0 ? void 0 : membersResult.data) !== null && _g !== void 0 ? _g : []} initialItineraryItems={itineraryItemsResult} initialSplitwiseGroupId={(_h = trip.splitwise_group_id) !== null && _h !== void 0 ? _h : null} initialSplitwiseExpenses={splitwiseDataResult.expenses} initialLinkedGroupName={splitwiseDataResult.groupName} 
    // Pass manual expenses down
    initialManualExpenses={manualExpensesResult} userRole={userRole} canEdit={canEdit} isTripOver={isTripOver}/>);
}
