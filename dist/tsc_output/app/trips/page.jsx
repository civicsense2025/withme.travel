...
const UserTripsSection = async () => {
    ...
    try {
        ...
        const combinedTripsMap = new Map();
        // Create a map of trips by ID for easy lookup
        tripsData.forEach(trip => {
            combinedTripsMap.set(trip.id, Object.assign(Object.assign({}, trip), { created_by: trip.user_id || user.id, role: 'unknown', memberSince: undefined // Default, will be updated
             }));
        });
        ...
        {memberTrips.length > 0 && (<div>
            <h2 className="text-2xl font-bold mb-4">Trips You're Invited To</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {memberTrips.map((trip) => (<TripCard key={trip.id} trip={trip}/>))}
            </div>
          </div>)}
      </div>);
    }
...
const TripsSectionSkeleton = () => (<div className="space-y-8">
    <h2 className="text-2xl font-bold mb-4">Your Trips</h2>
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      {[1, 2, 3].map((i) => (<div key={i} className="border rounded-lg p-4 space-y-3">
    ...
    try {
      ...
      `)
        ...
        <div className="flex justify-between items-center mb-6">
          ...
          <Button variant="ghost" asChild className="lowercase rounded-full hover:bg-travel-purple hover:bg-opacity-20">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          {templates.map((template, index) => {
                var _a;
                return (<ItineraryTemplateCard key={template.id} index={index} itinerary={{
                        id: template.id,
                        title: template.title,
                        description: template.description || "",
                        image: template.cover_image_url || ((_a = template.destinations) === null || _a === void 0 ? void 0 : _a.image_url) || "/placeholder.svg",
...
export default async function TripsPage() {
    return (<div className="container py-10">
      <div className="flex justify-between items-center mb-8">
        <PageHeader heading="Your Trips" description="Manage and organize your travel adventures"/>
