...
import { getDestinationPhoto } from '../utils/unsplash';
...
async function processDestination(destination) {
    ...
    try {
        const locationString = state_province ? `${city}, ${state_province}, ${country}` : `${city}, ${country}`;
        console.log(`Processing ${locationString}...`);
        const { photo } = await getDestinationPhoto(city, country, state_province, true);
