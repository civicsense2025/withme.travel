...
function createOnboardingPayload(userId, updates) {
    var _a, _b, _c, _d, _e;
    return {
        p_user_id: userId,
        p_first_name: (_a = updates.p_first_name) !== null && _a !== void 0 ? _a : null,
        p_travel_personality: (_b = updates.p_travel_personality) !== null && _b !== void 0 ? _b : null,
        p_travel_squad: (_c = updates.p_travel_squad) !== null && _c !== void 0 ? _c : null,
        p_onboarding_step: (_d = updates.p_onboarding_step) !== null && _d !== void 0 ? _d : null,
        p_complete_onboarding: (_e = updates.p_complete_onboarding) !== null && _e !== void 0 ? _e : false,
    };
}
async function testOnboardingFlow() {
    ...
    try {
        ...
        const { data: basicInfo, error: basicError } = await supabase.rpc('update_profile_onboarding', basicInfoPayload);
        if (basicError)
            throw basicError;
        ...
        const { data: personalityInfo, error: personalityError } = await supabase.rpc('update_profile_onboarding', personalityPayload);
        if (personalityError)
        ...
        const { data: squadInfo, error: squadError } = await supabase.rpc('update_profile_onboarding', squadPayload);
        if (squadError)
            throw squadError;
        ...
        if (!tagsData || tagsData.length !== tagSlugs.length) {
            const foundSlugs = (tagsData === null || tagsData === void 0 ? void 0 : tagsData.map(t => t.slug)) || [];
        ...
        const { data: completeInfo, error: completeError } = await supabase.rpc('update_profile_onboarding', completePayload);
        if (completeError)
            throw completeError;
