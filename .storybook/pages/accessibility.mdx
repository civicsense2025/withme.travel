...
- **Storybook a11y addon** - Runs axe-core tests on rendered components
- **ESLint jsx-a11y** - Catches common accessibility issues during development
- **Jest with testing-library** - Tests keyboard interactions and focus management

### Manual Testing

For comprehensive accessibility testing, we also perform manual checks:

1. **Keyboard navigation testing**

   - Tab through the interface and ensure all interactive elements are reachable
   - Check that focus order is logical and follows visual layout
   - Verify that keyboard shortcuts work correctly

2. **Screen reader testing**

   - Test with at least one screen reader (e.g., NVDA, VoiceOver)
   - Verify that all information is properly announced
   - Check that state changes are communicated appropriately
...
3. **Reduced motion testing**
   ...
   - Verify that essential functionality is still available
